...
| **B1b hardening** (`FIN-B1B-HARDENING`) | superseded by the row below | whitecross-site `ecd029ab` (claim `03d21f36`, released `b52dd26a`); 223/223 tests; introduced `REFUND_FEE_ACTUAL` entries, deterministic compensation ids and the **new-scope** Stripe Events backstop (plan §6.3.1, §3.1); its observation-ordering model was withdrawn the next day |
| **B1b recency** (`FIN-B1B-RECENCY`) | **SOURCE_READY_NOT_DEPLOYED** (2026-09-16) — **the candidate for a B1b-inclusive release** | whitecross-site `925debde` (claim `538d4723`, released `0e6de132`); 220/220 tests, race tests written first; fenced reconciliation (`booking.settlementRefundState` + generation fence), money stays immutable in `settlements/`; local rehearsal 8/8 (`evidence/fin-processor-fees/2026-09-16-b1b-local-rehearsal/`); release package `FIN_B1B_RELEASE_PREFLIGHT.md` |

...
| G10 | **Side finding, NOT part of EV2 backup alerting:** Cloud Run service `parsebookingemails` (us-central1) failed every ~5 min with `Error: invalid_grant` and a `POST 500` — ~8,125 ERROR entries in 14 days, 5xx-only for ≥6 weeks on the Cloud Run metric. Nothing alerted on it. **2026-09-26T16:12:43Z: scheduler job `firebase-schedule-parseBookingEmails-us-central1` PAUSED** (ledger `OPS-2026-09-26-E`, INCIDENTS 2026-09-26); service, source and Gmail secrets untouched. Permanent cleanup after one quiet week, each a separate approval and all before any whitecross Functions deploy (which would re-enable the job): remove the export from `whitecross-site/functions/index.js`, targeted function delete, Gmail secret cleanup, and the service's **public-access (no authentication)** setting (verified technical state in the Console; no exploit or customer effect claimed) reviewed as a security item | a legacy job with no owner and no success path; a dead OAuth client on a production project |
| G11 | **Side finding, NOT part of EV2 backup alerting:** `firebase-schedule-syncClientsToSheet-us-central1` (daily 01:06Z, Cloud Run `syncclientstosheet`) failed every run: `JSON.parse` of the `GOOGLE_SERVICE_ACCOUNT` secret throws *Bad control character in string literal at position 168* — a malformed secret value (version 1, never re-versioned), not an auth failure; 30/30 nights failed in the log window, 6-week metric 5xx only, no success line found. The source intends a Google Ads Customer Match export (every whitecross client's e-mail, phone, name to a Google Sheet) with **no opt-out/consent filter**; whether the Sheet is connected to Ads was not verified. **2026-09-26T18:12:22Z: job PAUSED** (same ledger row) as a privacy safeguard — repairing the secret would restart the export; no consent/opt-out filter was found in the source and no other legal basis or Ads linkage was verified within this audit, so a product/privacy decision is required before any restart. Service (public access — verified technical state only) and both secrets untouched; same cleanup path and deploy caveat as G10 | a broken job whose repair would move personal data before the consent/legal-basis question is answered |

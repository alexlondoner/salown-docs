...
## R-2026-09-08-C — `C1 BOOKING-PRICE-EDIT-AUTHORITY` browser cutover · 1-unit release (`hosting:salown`) from an isolated pinned workspace · **LIVE_VERIFIED (bytes) — production behaviour test NOT run**
...
| **Production business-data writes** | **ZERO.** No booking edited, no callable invoked, no Firestore document written. **C1 enforcement is now reachable by operators but has NOT been exercised on production** — the first real price-affecting edit is the live behaviour test and needs its own data-mutation approval |

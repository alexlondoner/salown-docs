...
> ---
>
> ### 📍 CURRENT — **2026-09-26 `R-2026-09-26-E` — `NEEDS-REASSIGN-ADMIN-UI`: bookings that still need a new team member are visible on the Admin day grid, detail panel and Home**: `hosting:salown` `2e322ecb469b09d3` → **`0388557370c99366`** (release `1790450736724000`, 19:25:36Z, source `8a6f8db` = live `f54f9c4` + `cherry-pick -x bb10b7c`; 32/32 served assets byte-identical to the isolated build; real content change only in `index` / `Home` / `Dashboard`, CSS identical). Rollback `2e322ecb469b09d3`. Gates: unit+render+parity 254/254, Home screen suite 3/3 (emulator), tsc 0, eslint 0; full vitest 206/209 with the 3 environmental reds named in the ledger. Read-only smoke limit: no qualifying unresolved booking existed in production to exercise the new UI. **`main` carries it as `bb10b7c`** (fast-forward, `[skip ci]`, no CI run, hosting unchanged after the push) — `main` == live on the 11 files except a 2-line import order. Staff UI still does not read `needsReassignment`. `hosting:salown-staff` `e2e82316f019037f`, ruleset `5e102dd4…`, functions unchanged.
>
> *previous CURRENT:* **2026-09-26 `R-2026-09-26-D` — `STAFF-RESCHED-SERVER` Staff half: the Staff Reschedule sheet moves a booking through `salownPatchBookingDetails`, no direct write**: `hosting:salown-staff` `d29ade1f6679bc30` → **`e2e82316f019037f`** (release `1790416449951000`, 09:54:09Z, source `08db336` = live `77e57dd` + 3 commits; served `staff-BRZuXB-d.js` 1,161,078 B byte-identical to the pinned build; `-B`'s prepaid markers intact). Rollback `d29ade1f6679bc30`. Staff gate writer 38/38 + screen 56/56. **`main` carries both halves since `8f40f86`** (five plain cherry-picks, `[skip ci]`, no hosting moved; claim released `09a85c5`) — `main` == live for the seven files. Whitecross REL-15 (`cc71f9bd`) integrated onto whitecross-site `main` as `de1cee79` the same hour (path filter + `[skip ci]`, no CI run). Owner production check of a real Staff move pending.
>
> *previous CURRENT:* **2026-09-26 `R-2026-09-26-C` — `STAFF-RESCHED-SERVER` Functions half: the booking-edit executor reads an ISO date and keeps `passive` absolute on a move**: `salownPatchBookingDetails` `-00004-xaf` → **`salownpatchbookingdetails-00005-qen`** (from `7c0e1ec` = live `5351a89` + 2 commits; deployed archive `src`/`lib` byte-identical to the workspace; config drift none; only this revision moved). Rollback `-00004-xaf`. **Staff hosting half NOT released:** the pre-deploy re-read found `hosting:salown-staff` on `d29ade1f6679bc30` (`77e57dd`, `R-2026-09-26-B`), not `601ff38`, so the owner's gate stopped it; candidate rebound as `release/staff-resched-on-live-77e57dd`, awaiting rebuild + gates + owner go. `main` does NOT carry `7c0e1ec` (executor on main == outgoing `5351a89`).
>
> *previous CURRENT:* **2026-09-26 `R-2026-09-26-A` + `-B` — `TREATWELL-PREPAID-WRITER`: a Treatwell prepayment survives checkout; an unestablished one is refused before payment**: `hosting:salown` `92846a10f16bbec8` → **`2e322ecb469b09d3`** (`f54f9c4`); `hosting:salown-staff` `4b34fd622c7abebe` → **`d29ade1f6679bc30`** (`77e57dd`). Rollbacks `92846a10f16bbec8` / `4b34fd622c7abebe`. ARTIFACT_VERIFIED (served 96/96 and 25/25 byte-identical); not screen-observed in production. ⚠️ **`main` does NOT carry it** — `review/treatwell-prepaid-writer-on-main` @ `7fa122d` unmerged; an Admin deploy from `main` would revert it. No Functions / rules / data change. **Update 2026-09-26 13:00 UK: `main` MERGED — `eab280b` (merge of `7fa122d`, `[skip ci]`, no deploy, no CI run, live ids unchanged); `main` now carries every 2026-09-26 Admin/Staff fix.**
>
...
>
> *Previous CURRENT line, kept:* **2026-09-25 `R-2026-09-25-B` + `-C` — Admin Home recorded-name pooling, then Clients spend/history**: `hosting:salown` `862cebeba11b1063` → `23e3739379e156f3` (B, `752395d` = live `464ee53` + Home) → **`5c56a953570b90e9`** (C, `45029ad` = B + Clients). Rollback C → `23e3739379e156f3`, B → `862cebeba11b1063`. Main carries both (`0cc630a`, `[skip ci]`, no CI release).
>
> *Previous CURRENT line, kept:* **2026-09-25 `R-2026-09-25-A` — Admin Home released**: `hosting:salown` `24930db8f0625b04` → **`862cebeba11b1063`** (`464ee53` = live `41b15ce` + Home with event-scoped Ignore). Rollback `24930db8f0625b04`. Main carries it (`b59973a`).
>
> *Previous CURRENT line, kept:* **2026-09-24 `R-2026-09-24-B` + `-C` — Admin A1 loyalty reader, then A2 Reports passive-barber**: `hosting:salown` `ef9aceae78492916` → `a6d11adb4593c912` (A1, `764ff84`) → **`24930db8f0625b04`** (A2, `41b15ce` = A1 + Reports). Rollback A2 → `a6d11adb4593c912`, A1 → `ef9aceae78492916`. Home still NOT released.
>
> *Previous CURRENT line, kept:* **2026-09-24 `R-2026-09-24-A` — STAFF-AVAILABILITY-UNTIL enforced on 12 functions + Staff + whitecross**: `availabilityUntil` inclusive, no override beats it; each function from a candidate on its OWN live source (see RELEASE_LEDGER). `hosting:salown-staff` **`9e7fafd49f8db5e8`** (`790f6c7`), `hosting:whitecrossbarbers-saas` **`1c5b7df0694ee139`** (REL-15, whitecross-site `cc71f9bd`), `hosting:salown` unchanged `ef9aceae78492916`. main carries the same content (`bb00254`, `66495e6`). ⚠️ Staff Reschedule's direct Firestore write is only UI-guarded (rules allow it); `needsReassignment` not shown to operators.
>
> *Previous CURRENT line, kept:* **2026-09-23 `R-2026-09-23-D/E/F` — CUSTOMER-GAP-STARTS LIVE on all three customer surfaces**: `salowncreatebooking-00006-mim` + `salowngetbusyslots-00065-dep` (from `af7b5ec` = live `3841b81` + change), `hosting:salown` **`ef9aceae78492916`** (from `af8347d` = live `1542ec9` + change), `hosting:whitecrossbarbers-saas` **`22137b7d0313a4d0`** (REL-14, anchor `a3b19bbe`). Rollbacks: `-00005-zer` / `-00064-foj` / `51bf2866e0af997e` / `c8eb6464e423073e`.
>
> *Previous CURRENT line, kept:* **`hosting:salown` RELEASED 2026-09-23 (`R-2026-09-23-C`, version `51bf2866e0af997e`) — ADMIN-PICKER-5MIN, served bundle byte-identical to the pinned build**; `functions:whitecross` as of 2026-09-22 (`R-2026-09-22-A`, both legs now production-verified); `hosting:salown-staff` as of 2026-09-21; `functions:salown` as of 2026-09-19; every other row is still as of 2026-09-09
>
> | Unit | Live now | Gap |
> |---|---|---|
> | `hosting:whitecrossbarbers-saas` | **`1c5b7df0694ee139`** (REL-15, `R-2026-09-24-A`, whitecross-site `cc71f9bd`; served 58/58 == REL-15 manifest; rollback `22137b7d0313a4d0`; **on whitecross `main` since `de1cee79`, 2026-09-26** — before that the live source lived only on `release/rel15-availability-until`) · *before:* **`22137b7d0313a4d0`** (REL-14, `R-2026-09-23-F`, released 2026-09-23T19:43:24Z, anchor whitecross-site `a3b19bbe`; rollback `c8eb6464e423073e`) · *before:* **`c8eb6464e423073e`** (REL-13, `R-2026-09-09-C`, released 2026-09-09T15:38:30Z) | none — supersedes `d310366f2e97bbfc` (REL-12, 14:43:31Z) |
> | `hosting:salown` | **`92846a10f16bbec8`** (`R-2026-09-25-F`, Home schedule on E, source `6091956`; rollback `c119f7a6d7780365`) · *before:* **`c119f7a6d7780365`** (`R-2026-09-25-E`, GRID-PRODUCT-SALE on D, source `d6312c6`; rollback `e385dead99e13249`) · *before:* **`e385dead99e13249`** (`R-2026-09-25-D`, Home note removed on C, source `05083a7`; rollback `5c56a953570b90e9`) · *before:* **`5c56a953570b90e9`** (`R-2026-09-25-C`, Clients on B, source `45029ad`; rollback `23e3739379e156f3`) · *before:* **`23e3739379e156f3`** (`R-2026-09-25-B`, Home recorded-name on `464ee53`, source `752395d`; rollback `862cebeba11b1063`) · *before:* **`862cebeba11b1063`** (`R-2026-09-25-A`, Home on `41b15ce`, source `464ee53`; rollback `24930db8f0625b04`) · *before:* **`24930db8f0625b04`** (`R-2026-09-24-C`, A2 `41b15ce` on A1 `764ff84`; rollback `a6d11adb4593c912`, then `ef9aceae78492916`) · *before:* **`ef9aceae78492916`** (`R-2026-09-23-E`, released 2026-09-23T19:32:30Z, from salown-app `af8347d` = live `1542ec9` + CUSTOMER-GAP-STARTS page; rollback `51bf2866e0af997e`) · *before:* **`8ad0529e5e7be72e`** (released 2026-09-22T14:43:41.264Z, `R-2026-09-22-B`, from salown-app **`daf1244`** = live `5be583c` + B2a + the F2 fix, branch `release/fin-b2a-only`, isolated-archive build with its own `npm ci`) | ✅ **LIVE_VERIFIED — `FIN-B2a`: the Stripe fee is finally visible on a booking.** Serves `/public-bundle/assets/index-BOx4e_RG.js`; **55/56 served text assets byte-identical** to the pinned build (the 56th is Firebase's auto-injected `init.js`); published file count **97 → 97**, 27 assets renamed 1:1, 0 added, 0 removed; exactly **one** chunk's content changed (entry, **+7,923 B**), CSS byte-identical. Served bundle carries `online-payment-fees`, `Awaiting Stripe`, `Remaining fee cost`, `Known fee cost`, `Charged more than once`, `Paid before fee tracking began`, `settledNetStatus` — **all absent from the previous bundle**, checked the same way before the deploy — and the F2 fix is visible in the minified reader (`EXTERNAL_CHECKOUT` no longer an evidence term). `/` `/app` `/book/**` `/login` 200; `/staff-bundle/**` still **302** to staff.salown.com, so `R-2026-09-20-A` is intact. **Authenticated UI pass done** (owner's browser, read-only): booking `jgyXnVAcMkQxdiNfsctO` reads `+£40.00 / −£0.80 / £39.20`; abandoned checkout `bjku8ZxN…` shows **no** Stripe block; a real historical Website payment **still** shows `Not recorded` (live negative control); 0 console errors. Rollback identity **`be573ea0498fc71f`** (release `1789909611896000`). Supersedes `be573ea0498fc71f` (`R-2026-09-20-A`). **`main` was not merged and did not move** (`55ec9df`) at the time of this release, so `main` lacked B2a and the F2 fix and an Admin deploy from `main` would have taken the fee block back off the screen. **Superseded 2026-09-23 — source only:** `MAIN-LIVE-RECONCILE` put the F2 fix on `main` (`origin/main` **`68dfab0`**), so that hazard is gone; see the 2026-09-23 section below. This row's live identity is itself superseded — the site is on **`102a5b6d0504adf5`** since 2026-09-23T00:20:55Z (`R-2026-09-23-B`) |
> | `hosting:salown-staff` | **`e2e82316f019037f`** (`R-2026-09-26-D`, release `1790416449951000`, 09:54:09Z, STAFF-RESCHED-SERVER: Reschedule through `salownPatchBookingDetails`, source `08db336` = live `77e57dd` + 3 commits; served `staff-BRZuXB-d.js` 1,161,078 B == build; rollback `d29ade1f6679bc30`) · *before:* **`d29ade1f6679bc30`** (`R-2026-09-26-B`, 00:19:42Z, TREATWELL-PREPAID-WRITER Staff, source `77e57dd` = live `601ff38` + 4 commits; rollback `4b34fd622c7abebe`) · *before:* **`4b34fd622c7abebe`** (`R-2026-09-25-G`, product-only sale off Today / Week grid / weekly summary, source `601ff38` = live `790f6c7` + 4 files; rollback `9e7fafd49f8db5e8`) · *before:* **`9e7fafd49f8db5e8`** (`R-2026-09-24-A`, from `790f6c7` = live `440e275` + availabilityUntil UI; rollback `e0fab0365adaca37`) · *before:* **`dc3e5984e1b9a63f`** (released 2026-09-20T22:53:09.483Z, `R-2026-09-20-B`, from **`bfb8116`** = live `76e58fe` + Staff loyalty/Welcome-Offer + the two writer guards, isolated-archive build with its own `npm ci`) | ✅ **RECORDED — Staff till loyalty redemption + Welcome Offer + apply-once and client-ownership guards.** Serves `/assets/staff-CuPJMecL.js`; **25/25 published files byte-identical** to the pinned build; served bundle carries `CHECKOUT_ALREADY_APPLIED`, `checkoutRevision`, `LOYALTY_CLIENT_MISMATCH`, `CheckoutLoyaltyClientMismatchError` and **0** emulator markers. Rollback identity **`4c0ce013b1e45dfa`** (release `1789679980588000`, 2026-09-17T21:19:34.687Z). **ARTIFACT_VERIFIED only** — no real Staff checkout observed through it yet. ⚠️ **This row was STALE:** it read `27b0c5187cb9976b` (`R-2026-09-15-A`, from `aa2efd9`) while the live site had already moved to `4c0ce013b1e45dfa` on 2026-09-17 — an unrecorded Staff release. The pre-deploy read for this release is what surfaced it. ⚠️ `main` does NOT carry `bfb8116` |
> | `functions:whitecross` (us-central1, **31**) | **2026-09-22 (`R-2026-09-22-A`, from whitecross-site `925debde`)**: `stripeWebhook` → **`stripewebhook-00107-yus`** (updateTime 2026-09-22T00:38:44Z; rollback identity **`stripewebhook-00106-dof`**, still listed and Ready) · `wcSettlementSweeper` → **`wcsettlementsweeper-00001-cat`, FIRST DEPLOY, no prior revision** + Scheduler job `firebase-schedule-wcSettlementSweeper-us-central1` `every 15 minutes` ENABLED. `enrollloyalty-00063-qil`, `wcloyaltylookup-00002-pec` unchanged. Inventory 30 → 31, Scheduler 4 → 5, nothing deleted | ✅ **RECORDED — FIN-B1 + FIN-B1b settlement ledger.** Deployed flag-off and proven inert first (8 consecutive sweeper passes `enabled: false … reason: 'DISABLED'`), then armed; first armed pass 11:08:08Z `enabled: true, configOk: true, refused: 0`, `scan.from` = `WC_SETTLEMENT_START_ISO` exactly. **`WC_SETTLEMENT_START_ISO = 2026-09-22T00:29:26Z` is permanent — never move it on a redeploy.** ⚠️ This deploy also republished the already-live `loyaltyEnroll.js` into the `stripeWebhook` bundle. ⚠️ **`wcSettlementSweeper` has no earlier revision — its only rollback is deletion.** **LIVE_VERIFIED 2026-09-22** — *upgraded from `PARTIALLY LIVE_VERIFIED` the same day:* the capture leg no longer waits. Booking `tenants/whitecross/bookings/jgyXnVAcMkQxdiNfsctO` carries two real ledger entries written by `stripe-webhook` at **2026-09-22T12:23:43.459807Z** — `stripe:ch_3UISjRRfgDnpYJzP1XFltzNC` `CAPTURED` `gross_m 4000` and `stripe:txn_3UISjRRfgDnpYJzP1C41nC0B` `FEE_ACTUAL` `fee_m 80` (gbp), `settlementSync: done, attempts 0, lastError null`, projection `feeSource: 'actual', feeComplete: true, providerNet_m: 3920`. £0.80 on £40.00 = Stripe UK 1.5% + 20p exactly, and the fee completed on the `checkout.session.completed` webhook itself — the indexed due pass was not needed for this payment. Read-only verification, `FIN_B2A_RELEASE_PREFLIGHT.md` §4.2/F6 |
> | `functions:salown` (europe-west2, **94**) | **2026-09-26 (`R-2026-09-26-C`)**: `salownPatchBookingDetails` → **`salownpatchbookingdetails-00005-qen`** (from `7c0e1ec` on live `5351a89`; ISO date + passive rung on a move; rollback `-00004-xaf`; `salownEditBookingForm` untouched at `-00002-muv`) · *before:* **2026-09-24 (`R-2026-09-24-A`)**: 12 functions redeployed, each from its own candidate — `salowncreatebooking-00007-hap`, `salowncreateadminbooking-00003-her`, `salowncreatewalkin-00003-jus`, `salowncreatestaffbooking-00002-xad`, `salowncreatestaffwalkin-00002-jom`, `salowneditbookingform-00002-muv`, `salownpatchbookingdetails-00004-xaf`, `salownreassignbooking-00004-vob`, `salownreschedulebytoken-00077-noc`, `salownparseemails-00131-rap`, `salownparseinboxdispatch-00014-quq`, `salownmanualimport-00120-piv` · *before:* **2026-09-23 (`R-2026-09-23-D`, from `af7b5ec` = live `3841b81` + change)**: `salownCreateBooking` → **`salowncreatebooking-00006-mim`**, `salownGetBusySlots` → **`salowngetbusyslots-00065-dep`** (rollback `-00005-zer` / `-00064-foj`) · *before:* **2026-09-19**: `salownSendLoyaltyEmail` → **`salownsendloyaltyemail-00067-bem`** (`R-2026-09-19-E`, from `c67da9e` = `c8a64d6` + receipt tender label; deployed zip byte-identical; rollback `-00066-cej`; ARTIFACT_VERIFIED). **NEW 2026-09-15**: `salownCreateStaffWalkIn` (`R-2026-09-15-A`, `aa2efd9`) — first deploy, no prior revision, `ACTIVE`, entry point + region confirmed, live unauthenticated call returns this codebase's own `UNAUTHENTICATED` reason. `salownCreateStaffBooking` re-read same session: **still** revision `salowncreatestaffbooking-00001-jan` (`R-2026-09-13-A`, `797c9b3`), unchanged — its source has since been refactored on `main` but that refactor is not live. Also live: 8 email functions on `R-2026-09-12-A` (`c8a64d6`): `salownbookingconfirmationtrigger-00046-qem`, `salownbookingconfirmedemailtrigger-00044-cas`, `salownnotifybookingupdated-00111-yog`, `salownsendbookingconfirmation-00109-luq`, `salownsendcancellationemail-00104-hib`, `salownsendloyaltyemail-00066-cej`, `salowncancelbytoken-00070-moq`, `salownreschedulebytoken-00076-mot` | ⚠️ B7 WhatsApp source on main, never deployed. `CUST-LANG-AUDIENCE` is **ARTIFACT_VERIFIED** (the deployed build, required in node with herohairs' real stored presentation, renders English subject/labels and an UNCHANGED `· 11:30` clock and `£35.00`) but **END-TO-END UNPROVEN** — the one live send used the legacy never-localised `salownSendBookingConfirmation` callable and proves nothing about the trigger path. `salownCreateStaffWalkIn`/`salownCreateStaffBooking` are reachable but **not the ONLY path for their respective flows** — `firestore.rules`' `isTenantAny` bypass and the legacy Admin `salownCreateWalkIn` callable still let writes skip every check these releases ship (both named, standing exceptions, not closed). Earlier rows (`salownSetStaffRole` `-00001-wam`, `ONB-INVITE`) unchanged by this release |
> | Firestore **rules** | ruleset **`5e102dd4-e7e7-4950-b12a-14a74daa82e8`** (release 2026-09-10T13:39:16Z, `R-2026-09-10-C`, source `34f64af`) — **re-read 2026-09-14**: 80,896 B, sha256 `b05ac1e7515cb5a5…`, byte-identical to salown-app `HEAD:firestore.rules` (`b6c325c`); `coaNotOverAllocated` ×2, `settlementLedgerEnabled` ×5, `autoRefundEnabled` ×6 | none. *Corrected 2026-09-14: this cell still said `a0a10819-…` with `[COA]` and the B1 flag arms PUSHED_NOT_LIVE; both went live in `R-2026-09-10-C`.* **Unchanged by `R-2026-09-22-A`** — re-read before and after the index deploy, still `5e102dd4-…` / 2026-09-10T13:39:16Z. **The B1 flag value is no longer absent: `settlementLedgerEnabled: true` since 2026-09-22T11:06:54Z** (audit `settlementflag_1790075214487`). `autoRefundEnabled` was disarmed 06:33Z → re-armed 11:13Z across the arm; it is `true` now |
> | Firestore **indexes** | **3, all `READY`** (2026-09-22). New: `bookings (settlementSync.state ASC, settlementSync.nextAttemptAt ASC)` COLLECTION, id **`CICAgJjF9oIK`**, deployed in `R-2026-09-22-A` from salown-app `f928117`, READY at 2026-09-22T00:28:23Z | ✅ **RECORDED.** The deploy created 1 and deleted 0, with no deletion prompt — the live `stripePaymentIntent` field override already matched the file. File and live are now in step: `firestore.indexes.json` declares exactly these 3 + the override. The index is inert on its own; it exists to serve `wcSettlementSweeper`'s due pass |
>
...
| `hosting:salown` | **`82a0e20`** (`release/admin-picker-on-live`, = live `37506d6` + the picker change ONLY; message-amend twin `1542ec9`, identical tree `ac0d173`) | version **`51bf2866e0af997e`**, release `1790175424010000`, 2026-09-23T14:57:04Z (`R-2026-09-23-C`, ADMIN-PICKER-5MIN) | version **`102a5b6d0504adf5`** (→ `R-2026-09-23-B`) |

**`R-2026-09-23-C` — ADMIN-PICKER-5MIN.** Both Admin booking pickers stepped 15 minutes while the Staff App walk-in picker steps 5, so an imported booking ending off the grid (a Booksy job 17:30–18:05 on Alex's chair, 2026-09-22) made the only bookable gap unreachable from Admin; `salownCreateAdminBooking` returned five HTTP 400 `SLOT_CONFLICT` between 15:42:17Z and 15:44:33Z and the front desk saw the raw machine code. **Provenance was proven before anything was built:** `37506d6` rebuilt in a clean `git archive` workspace produced `index-CaRkTTAn.js` and `BookingForm-Cip6QIjT.js` **byte-identical** to what production was serving, and `offGrid` was absent from the live chunk — so the change was definitively not already live. **`main` was deliberately NOT the base:** a build from `main` adds 13 Staff-App dictionary strings the Admin site has never shipped. The only UI-copy difference between live and the candidate is the one new `SLOT_CONFLICT` message; the "only in live" set is empty and the bundle grew 175 B. Gates on `82a0e20`: canonical emulator **645/645 PASS** (the `functions/` diff against the base is empty, so that figure is the base's own), `tsc` 0, frontend 16 fail on the candidate **and** 16 on the base in the same clone with `+30` passing, eslint `src` delta 0. After deploy `hub.salown.com/app` serves `index-C2tYoONR.js`, refetched and **byte-identical** to the pinned build. **Rollback `102a5b6d0504adf5`.** Scope held: `hosting:salown` only — no Functions, rules, indexes or Staff hosting, and no production booking or payment write. Source is on `main` as `5354c6f` (push `68dfab0..9620e90`, both commits `[skip ci]`, CI did not fire). The pre-deploy UI pass was run against an **isolated dev server on `http://localhost:5199`**, not production.
...
**`origin/main` was `a85bcf1` and carried NEITHER.** This work was deployed from branches on the owner's instruction: `main` did not contain the live Admin B2a/F2 source, so an Admin release built from it would have reverted that fix. The push-vs-live gap this file exists to track was therefore **deliberate and open**.

**2026-09-23 — that gap is now CLOSED ON THE SOURCE SIDE, and only there.** `MAIN-LIVE-RECONCILE` (owner-approved, source only) took `origin/main` `6937314` → **`68dfab0`**: the availability feature restored by cherry-picking `a82b5bd` (the revert-of-the-revert, and the exact source of `salownstafflifecycle-00004-saz`), the F2 fix by cherry-picking `daf1244`, then the two live repairs as their own deltas (`57aba20..6c55dc2`, `6c55dc2..37506d6`). No branch was merged blind: B2 (`74922bd`) and B2a (`9a4925c`) were already on `main` and were not re-applied, and `main`-only work (parsers, `cancellationCopy`, `onlinePrepaymentEdit`, `platformDepositEdit`, `staffPolicyGate`, `paymentAllocation`, the Staff override flow, Reports passive-barber, `bookingCallables`) is preserved. Gates on the integrated head: `tsc` clean both projects, frontend 5782/3 (2 machine-state + 1 worktree-topology artefact of `ops/rules-authority.test.js`), functions 2915/0/48, canonical emulator **732/732** against a 725 base, both builds clean. **Nothing was deployed and nothing about what is live changed** — `hosting:salown` is still `102a5b6d0504adf5` (`index-CaRkTTAn.js` re-read from the live page after the push) and `salownStaffLifecycle` still `-00004-saz`. `main` continues to carry work that has never been released, so *pushed* still does not mean *live*; the Admin 5-minute picker is **no longer** out of scope: it shipped the same day as `R-2026-09-23-C` (`51bf2866e0af997e`) from the live source `37506d6`, and its source is on `main` as `5354c6f`. Detail: `salown-app` `SYNC.md`, entry `MAIN-LIVE-RECONCILE`.
...
moves the appointment by the offset on every BookingForm save (and on a panel date/time change). UK tenants unaffected. `4882d56`
converts through the instant with the presentation contract's zone. **Do not run the BookingForm live test until this is released**
(separate `hosting:salown` approval; candidate `4882d56`, rollback `cde349264ff973c8`).

*(previous header)* PARTIALLY_LIVE_VERIFIED 2026-09-08 — patch callable + panel proven live; form callable + form adapter LIVE_UNPROVEN

...
> | `hosting:salown` | **`066e59717514985f`** (released 2026-09-10, `R-2026-09-10-F`) | ✅ **RECORDED** — A5 T-e path 3 panel half (`649192b`) + `STAFF-CLIENT-CREATE` panel half (`abd29e4`+P1), from `3c2603f`, isolated-workspace build, byte-verified. Superseded chain today (not individually re-listed here): `R-2026-09-10-B` → `-F`. ⚠️ **`BEHAVIOUR_UNPROVEN`** — no owner role-change or panel duplicate-refusal exercised yet. `hosting:salown-staff` confirmed UNCHANGED at `d0ec217095bdda67` by this release (REL-1 side-build did not leak) |
...
> | `functions:salown` (europe-west2, **92**) | `salownSetStaffRole` **`-00001-wam`** (new create, 2026-09-10T16:4xZ, `R-2026-09-10-E`, from `304c036`) | ⚠️ B7 WhatsApp source on main, never deployed. `ONB-INVITE` welcome-email copy is **ARTIFACT_VERIFIED** (new text confirmed inside the deployed source zip) but **BEHAVIOUR_UNPROVEN** — no approval has run since. New function is **ARTIFACT_VERIFIED (function exists, 91→92)**, **BEHAVIOUR_UNPROVEN** — the panel (`hosting:salown` `066e59717514985f`) now opens the calling UI (Settings → Staff), so this callable is reachable end-to-end; no owner has exercised it in production yet |

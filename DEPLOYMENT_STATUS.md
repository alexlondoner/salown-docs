...
> ---
>
> ### 📍 CURRENT — 2026-09-09, one screen · **`hosting:salown-staff` + `functions:salown` rows re-read live 2026-09-15 after `R-2026-09-15-A`; `hosting:salown` re-read live 2026-09-15 as a negative control (unchanged since `R-2026-09-12-A`); every other row is still as of 2026-09-09**
>
> | Unit | Live now | Gap |
> |---|---|---|
...
> | `hosting:salown` | **`827946e295c69eeb`** (released 2026-09-12T14:41:44Z, `R-2026-09-12-A`, from `c8a64d6`, isolated-workspace build) | ✅ **RECORDED** — `CUST-LANG-AUDIENCE`: the public booking page and profile no longer read in the OWNER's panel language. Served bundle `index-4wSzOuqQ.js` sha256 `7560d8d7…` byte-identical to the pinned build; `/book/herohairs` DOM verified all-English live (`Choose a service`, `From £35.00`, `Please choose an option.`). Supersedes `c389253b23bb981b` (`R-2026-09-11-B`), which itself superseded the `066e5971…` row below — this row was NOT re-verified for the 09-10/09-11 releases when they happened. **Re-confirmed UNCHANGED 2026-09-13** after the `hosting:salown-staff` deploy below (its predeploy hook fired but nothing was uploaded) |
> | `hosting:salown-staff` | **`27b0c5187cb9976b`** (released 2026-09-15T12:57:58Z, `R-2026-09-15-A`, from `aa2efd9`, isolated-workspace build) | ✅ **RECORDED — Staff App Walk-in flow protections published (STAFF-AVAIL-GAP-P2 + WALKIN-BACKDATE-FLOOR). Scope claim: Staff App Walk-in flow ONLY — not tenant-wide, not zero-regression, not all-gaps-closed.** Served bundle `staff-CxdWlU6-.js` sha256 `ee92f6df…` + `staff-h5sE0F85.css` sha256 `01286319…` byte-identical to the pinned build; source markers confirm the old `9 * 60` floor is absent and `startTimeIso`/`salownCreateStaffWalkIn` are present. Supersedes `62aa1ac4a0302593` (`R-2026-09-13-A`). Also ships New Booking's refactored CLIENT code against its UNCHANGED server (`salownCreateStaffBooking` — see the functions row); compat-tested 13/13, `RELEASE_LEDGER.md` `R-2026-09-15-A`. ⚠️ `firestore.rules`' `isTenantAny` bypass and the legacy Admin `salownCreateWalkIn` bypass are UNCHANGED — both **named, standing exceptions**, not closed. Walk-in↔Reschedule race deferred to Phase 3, no system-wide conflict guarantee. Block Time (Phase 4) not started |

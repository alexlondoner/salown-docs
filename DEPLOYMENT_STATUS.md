...
> ---
>
> ### 📍 CURRENT — **`hosting:salown` re-read live 2026-09-20 after `R-2026-09-20-A`; `hosting:salown-staff` re-read the same day as a negative control (unchanged)**; `functions:salown` as of 2026-09-19; every other row is still as of 2026-09-09
>
> | Unit | Live now | Gap |
> |---|---|---|
...
> | `hosting:salown-staff` | **`27b0c5187cb9976b`** (released 2026-09-15T12:57:58Z, `R-2026-09-15-A`, from `aa2efd9`, isolated-workspace build) | ✅ **RECORDED — Staff App Walk-in flow protections published (STAFF-AVAIL-GAP-P2 + WALKIN-BACKDATE-FLOOR). Scope claim: Staff App Walk-in flow ONLY — not tenant-wide, not zero-regression, not all-gaps-closed.** Served bundle `staff-CxdWlU6-.js` sha256 `ee92f6df…` + `staff-h5sE0F85.css` sha256 `01286319…` byte-identical to the pinned build; source markers confirm the old `9 * 60` floor is absent and `startTimeIso`/`salownCreateStaffWalkIn` are present. Supersedes `62aa1ac4a0302593` (`R-2026-09-13-A`). Also ships New Booking's refactored CLIENT code against its UNCHANGED server (`salownCreateStaffBooking` — see the functions row); compat-tested 13/13, `RELEASE_LEDGER.md` `R-2026-09-15-A`. ⚠️ `firestore.rules`' `isTenantAny` bypass and the legacy Admin `salownCreateWalkIn` bypass are UNCHANGED — both **named, standing exceptions**, not closed. Walk-in↔Reschedule race deferred to Phase 3, no system-wide conflict guarantee. Block Time (Phase 4) not started | · **negative control 2026-09-20: unchanged after `R-2026-09-20-A`** — live version re-read as `4c0ce013b1e45dfa` (release `1789679980588000`, 27 files), `staff.salown.com` still serves `staff-BVUJwYTL.js`. *(Note: this row's `27b0c5187cb9976b` is older than the live version id read on 2026-09-17/09-20; the live identity of record for this site is `4c0ce013b1e45dfa`.)*
> | `functions:whitecross` (us-central1, 30) | `enrollloyalty-00063-qil`, `wcloyaltylookup-00002-pec` current | ⚠️ **FIN-B1 functions absent** (re-read 2026-09-14: no `wcSettlementSweeper` in functions, Cloud Run or Scheduler; `stripeWebhook` still `stripewebhook-00106-dof`, bundle byte-identical to whitecross-site `6817356f`). FIN-B1's rules arm is live — see the rules row |
...
> | Firestore **rules** | ruleset **`5e102dd4-e7e7-4950-b12a-14a74daa82e8`** (release 2026-09-10T13:39:16Z, `R-2026-09-10-C`, source `34f64af`) — **re-read 2026-09-14**: 80,896 B, sha256 `b05ac1e7515cb5a5…`, byte-identical to salown-app `HEAD:firestore.rules` (`b6c325c`); `coaNotOverAllocated` ×2, `settlementLedgerEnabled` ×5, `autoRefundEnabled` ×6 | none. *Corrected 2026-09-14: this cell still said `a0a10819-…` with `[COA]` and the B1 flag arms PUSHED_NOT_LIVE; both went live in `R-2026-09-10-C`.* The B1 flag *value* is still absent from `tenants/whitecross/settings/settings` (inert) |
> | Firestore **indexes** | 2, both `READY` (**re-read 2026-09-14**) | ⚠️ B1 `settlementSync` index in file, not deployed — belongs to the FIN-B1 release (`FIN_B1_RELEASE_PREFLIGHT.md`) |
>

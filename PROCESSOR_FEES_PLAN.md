...
### 3.1 Refund statuses and the Events API — two evidence tiers, deliberately not merged
...
**Evidence tiers (`FIN-B1B-RECENCY`, 2026-09-16/17).** The implementing session has no network access, so what it
could and could not check itself is kept apart:

| Tier | What | How it was checked |
|---|---|---|
| **A · locally verifiable** | the five `status` values · `cancel()` works **only** from `requires_action` and "only refunds for payment methods that require customer action can enter the `requires_action` state" · `pending_reason` ∈ `charge_pending`/`insufficient_funds`/`processing` · refunds list "most recent first" · the refunds resource has `create/retrieve/update/list/cancel` and **no `delete`** · Events API `types` (≤ 20, exclusive with `type`), `limit` 1–100, `created` range, reverse-chronological pagination, 30-day retention, per-event `api_version` | read from the vendored SDK type definitions (`stripe` 18.5 `types/*.d.ts`), which carry the API-reference text verbatim |
| **B · coordinator-fetched, NOT re-verifiable here** | every **transition edge** in the table below, in particular `succeeded → requires_action` and `succeeded → failed` | fetched from the doc pages by the coordinator and relayed; the implementing session could not open them |

**Rule that follows:** no correctness claim about an ordinary flow may rest on tier B. The implementation therefore
uses the transition table **only** as a health check that can trigger a re-read and, if a contradiction persists, a
review item. Recency comes from the generation fence (§2.4), which does not consult a status at all. If a tier-B edge
turns out to be wrong, the effect is a spurious re-read and possibly a review flag — never a wrong `refunded_m`,
never a silently "complete" booking. Tier B is also why `pending → canceled` is flagged below: it contradicts the
tier-A `cancel()` statement, and that contradiction is left visible rather than resolved by guesswork.
...
⚠️ **`pending → canceled` (tier B) contradicts tier A**, which says `cancel()` refuses every status but
`requires_action`. Both readings are recorded; the code does not depend on either, because a `canceled` refund is
handled by what the listing shows, not by how it got there.

⚠️ **"The SDK has no `delete` method" is not a listing-consistency guarantee.** It is the reason a refund id is not
expected to disappear once seen, and nothing more: the pagination docs say nothing about consistency while objects
are being created, and no page states that a complete listing always contains every refund of a charge. So a refund
id that an event names and a fresh complete listing does not show is treated as a **contradiction to re-read**
(bounded, then review) — never as proof that the refund was deleted, and never as a reason to drop a recorded
refund.
...
- **Since `FIN-B1B-RECENCY`:** the backstop reconciles through the same per-charge generation fence as every other
  writer (§2.4), so a lost-webhook recovery can never overwrite a fresher reconciliation, and a fenced backstop
  attempt writes nothing and leaves the work to the due pass.
...
| B1b | Refund entries — **`SOURCE_READY_NOT_DEPLOYED`; the candidate is now whitecross-site `925debde` (`FIN-B1B-RECENCY`, 2026-09-16), which supersedes `95a963fe` (2026-09-15) in source before any deploy**, not part of the B1 release pinned at `22850996`. The claim was handed over with owner approval (`ef17f1f6`) and extended to `index.js` (`977062bb`); released `3668c506`. What landed:<br>• `REFUNDED` entries `stripe:re_<id>` for succeeded refunds; the refund fee lives in its own `REFUND_FEE_ACTUAL` entry and an unknown fee is never 0 (§2.4, §2.5).<br>• Provider timestamps stored as facts only; no refund or fee day is chosen.<br>• Routing: `charge.refunded` / `refund.updated`. Before this, the BL-5 refund block in `index.js` answered 200 and returned before the settlement call, so refunds never reached the ledger. Now a guarded hook runs before each 200; reconcile behaviour is unchanged.<br>• Superseded ids and rules from the first two attempts (the `amount_refunded` snapshot on entries, `REFUND_OBSERVED`, `comp:re_<id>:fee_known`, the `stripe:re_<id>:<txn>` replacement) are recorded in §2.4 as withdrawn; none was ever released.<br>• **Known limits:** the `amount_refunded` observation is refreshed only when an entry is written, so a lost refund webhook on a booking already done is caught only by redelivery or a later capture-path run. There is no provider-side refund scan. Whether Stripe's `amount_refunded` counts pending refunds is not verified on the live API.<br>• **Any release needs a new rehearsal and preflight.**<br>• **NEW SCOPE (owner, 2026-09-15): `FIN-B1B-HARDENING`**, which is not part of B1 or of the original B1b. (1) Refund observations become their own append-only entries, so an `amount_refunded` change is not lost while a refund entry already exists. Conflicting observations follow an explicit pairwise-dominance rule, not a clock or a rank sum. (2) Late refund-fee and correction ids are reconciled with the append-only contract. (3) **The Stripe Events API backstop scan in `wcSettlementSweeper` is new scope.** It rescans refund events in a window over a persisted cursor. The cursor never moves past unprocessed events on an error or rate limit. The same kill switch, account and livemode gates apply. A retention gap is recorded, never skipped silently. It adds a new Stripe API use (`events.list`) to a release. Contract changes and the recovery limits are recorded in §2.4/§2.6 once the code lands.<br>• **`FIN-B1B-RECENCY` (2026-09-16, whitecross-site `925debde`) replaced the recency model before any deploy.** `REFUND_OBSERVED` is withdrawn and was never released. Refund recency is a per-charge `generation` fence on the mutable `booking.settlementRefundState` (§2.4); the transition table is only a health check (§3.1, tier-B evidence); an ordinary `pending → succeeded` completes with no review; a fenced writer writes **nothing**, retries from a fresh read, and then hands the work to the sweeper with a `REFUND_FENCED` marker. Money stays in immutable entries; the snapshot is only the "as of" record.<br>• **Gates run:** `cd functions && npm test` **220/220** at `925debde` (includes B1 parity against `22850996` and the salown-app `readProjection` contract); **local Firestore emulator rehearsal 7/7** (fake Stripe, project `demo-b1b-recency`) covering both writer-race commit orders, a parallel race, fencing after an unchanged snapshot, retry exhaustion → sweeper takeover, two charges keeping both totals, and `pending → succeeded` with no human step — recorded in [`TESTS.md`](TESTS.md) §0-B.<br>• **No new Firestore query or index:** the query set is identical to `22850996`.<br>• **Release conditions still open:** a B1b rehearsal on staging with real Stripe test-mode deliveries; the live endpoint's `charge.refunded` / `refund.updated` subscription confirmed; a B1b preflight of its own (the existing preflight covers `22850996` only); owner approval for the new `events.list` use; and the B1 release itself, which stays pinned at `22850996` and is unchanged by this work. | `REFUNDED` entries from the existing `charge.refunded` reconcile (`index.js` BL-5 block) | B1 | same unit | same |

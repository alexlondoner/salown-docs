...
konuşur: `BookingStatus`, `BookingSource`, `PaymentType`, `CampaignType`, `CouponType`,
`LoyaltyAdjustmentReason`, `EmailEventType`, `StripePaymentMode`. Bunlar bugün string
literal olarak dağınık (casing bug'ının kaynağı) → tek union type = compile-time koruma.

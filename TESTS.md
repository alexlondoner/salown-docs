...
## 0-D. FIN-B1b refunds (`FIN-B1B-RECENCY`, candidate whitecross-site `925debde`) — 2026-09-17

| Work | Gate | Command / where | Result | Durable evidence | What it does NOT prove |
|---|---|---|---|---|---|
| **B1b** | unit (node:test) | `cd whitecross-site/functions && npm test` at `925debde`, **in a checkout with git history AND a sibling `salown-app`** | **220 / 220 pass, 0 skipped** (settlements 83 · webhook integration 8 · refunds 40 · externalCheckout 73 · loyalty 16). ⚠️ **Re-measured 2026-09-21:** in a `git archive` workspace the same command gives **219 pass + 1 SKIP** — the B1 parity test cannot reach `22850996` — and the reader test's live half returns early without a skip. Read the **skip count**, not only the pass count; `0 skipped` is part of the gate. Method in `FIN_B1B_RELEASE_PREFLIGHT.md` §3 | the suite files `settlements.test.js`, `stripeWebhook.integration.test.js`, `settlements.fakes.js` | anything about real Stripe or production |
| **B1b** | B1 parity | inside the run above | a booking that never had a refund is **byte-identical** to `git show 22850996:functions/settlements.js` (entries, projection, marker, scan cursor). **Runs only when `22850996` is reachable from the checkout** — otherwise the test skips itself and the gate is not measured | the parity test loads the pinned source itself | that B1b is safe to deploy — B1 stays pinned at `22850996` |
| **B1b** | reader contract | inside the run above | every projection written passes the pinned copy **and** the live `salown-app/src/utils/settlementFacts.ts` `readProjection`. **Without that sibling path the live half returns early and the test still PASSES** — a silent degradation, not a skip | the reader test imports the real TS file when salown-app is present | Finance UI behaviour (B2) |
| **B1b** | **local Firestore emulator rehearsal** | `firebase emulators:start --only firestore --project demo-b1b-recency` (port 8099) + `node rehearse.js`; Stripe entirely faked | **7 / 7 scenarios pass** — ① writer race, fresh-read commits first: the stale worker is fenced and writes nothing ② the other order: the newer worker is fenced and its work is redone from a fresh read ③ a genuinely parallel race: exactly one reconciliation wins, the refund is recorded once ④ fencing after an **unchanged** snapshot (the generation still advances) ⑤ retry exhaustion: no partial write, `REFUND_FENCED` marker, then the sweeper's due pass completes it ⑥ two charges on one booking: independent generations, `gross_m` 6400 and `refunded_m` 1500 both kept ⑦ `pending → succeeded` completes with **no** review flag and no human step | committed: [`evidence/fin-processor-fees/2026-09-17-b1b-fence-rehearsal/`](evidence/fin-processor-fees/2026-09-17-b1b-fence-rehearsal/) (script, output, emulator config). A second, scenario-level rehearsal of the same candidate — webhook, lost-webhook backstop recovery, pending/cancel/fail and the kill switch — is in [`evidence/fin-processor-fees/2026-09-16-b1b-local-rehearsal/`](evidence/fin-processor-fees/2026-09-16-b1b-local-rehearsal/) | real Stripe (all Stripe calls are the fake), webhook HTTP delivery, the composite index, the scheduler, live mode, or any production behaviour |

| **B1b** | **API-read rehearsal** (the two NEW Stripe reads) | `whitecross-site/ops/rehearsal/api-readonly/`, `emulators:exec --config firebase.json.rehearsal-example --only firestore --project demo-salown-b1b 'node api-rehearse.cjs'` | **12 steps, 52 / 52 checks, exit 0** — 291 Stripe calls, **all reads, 0 write attempts**. `events.list` bounds (`types[]` = the four backstop types, `limit` 100, one closed `created` window per pass), `refunds.list` with `expand[]=data.balance_transaction` across pages, an unending list refused, rate limit / rejected resume / retention gap, kill switch = 0 calls. **A0 is the negative control:** `refunds.create` through the same guard is blocked and recorded | committed: `whitecross-site/ops/rehearsal/api-readonly/` (guard, scenario, README, `evidence/2026-09-21-925debde-api-readonly.jsonl`) | real Stripe of any kind — no test-mode call has been made; webhook HTTP delivery; index enforcement; the scheduler |
| **B1/B1b** | flag arming tool | `cd whitecross-site && node --test scripts/activate-settlement-ledger.test.mjs` | **29 / 29 pass** — dry run writes nothing; a stale `--expect-update-time` is refused by the tool AND by the server precondition; arming fails CLOSED when the due-pass index is missing, CREATING, NEEDS_REPAIR or unreadable; arming while `autoRefundEnabled` is true needs `--ack-auto-refund-armed` and records it; **disarming is never blocked**; one commit, one field, a real boolean; the tool carries no Firestore deploy invocation | `scripts/activateSettlementLedger.mjs` + its suite; gcloud shadowed, global `fetch` replaced by an in-memory Firestore that enforces the preconditions itself | that the tool has ever been pointed at production — **it has not, not even a dry run** |

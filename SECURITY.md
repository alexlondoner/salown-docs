...
| Phase 1 | Single canonical rules file (`salown-app/firestore.rules`) | ✅ **DONE** (2026-06-21) — *as a convention*. See `RULES-SSOT` below: it was enforced by nothing until 2026-08-16 | — |
| **`RULES-SSOT`** | **Rules deploy AUTHORITY — one publisher, enforced** | ✅ **DONE in source, 2026-08-16** (`FIRESTORE-RULES-SSOT-P0`). Five non-canonical directories could publish a ruleset to `havuz-44f70`; three carried `match /{document=**} { allow read, write: if isAuth(); }` and a git worktree carried one with `allow read: if true` on every tenant subcollection. All `firestore` declarations removed, stale copies renamed `*.LEGACY-DO-NOT-DEPLOY.txt`, both stale worktrees pruned. Enforced by `salown-app/ops/rules-authority.test.js` (`npm test` + deploy workflow) and `whitecross-site/scripts/check-rules-authority.sh` (wired into `deploy.sh` + `deploy-functions.sh`). **No rules were deployed** — this changed who *may* publish, not what *is* published | 🔴→✅ |
...
| **G5** | Single global ruleset blast radius | ⚠️ Partial — deploy discipline is now **machine-enforced** (`RULES-SSOT`, 2026-08-16: one publisher, provable), but the blast radius itself is unchanged: one ruleset still governs every tenant, so a bad *deploy from the right place* is still platform-wide. No structural fix | 🟠 Medium |

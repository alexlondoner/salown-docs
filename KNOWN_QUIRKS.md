...
| **A missing source marker in a frontend bundle is NOT proof that code wasn't deployed** — the build eliminates identifiers *and some string literals*, so a marker can be absent from a bundle that genuinely contains the feature | Vite/rollup minify + tree-shake; a module behind a literal-const feature flag (`FINANCE_PERIOD_CLOSE_MODE = 'legacy'`) loses the branches that carry its distinctive strings | **Always run the negative control: grep the LOCAL build of the same commit.** If the marker is absent there too, the method is inconclusive for that marker — pick a different probe (behavioural check, or a marker you have confirmed survives). Proven 2026-09-09: `FROZEN` / `financePeriods` / `salownCloseFinancePeriod` = 0 hits in the served `Finance-COuJmy09.js` **and** 0 in the local build. Marker verification stays the right default for **functions** (server source is not minified this way) and for hosting when the marker is pre-verified | this file · [ROADMAP](ROADMAP.md) `FIN-PERIOD-CLOSE-C` |

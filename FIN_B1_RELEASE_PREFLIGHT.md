...
| Kill switch `tenants/whitecross/settings/settings.settlementLedgerEnabled` | **absent** (field-mask read); owner-only in the live rules | set boolean `true` at the arming step only. ~~**No activation tool exists**~~ — **superseded 2026-09-21:** `whitecross-site/scripts/activateSettlementLedger.mjs` (`c2f29b6f`, 29 tests, never run against production) is that guarded script — dry run by default, `updateTime` CAS, exact-field diff, flag + audit row in one commit — and it additionally refuses to arm while the due-pass index is not READY or while `autoRefundEnabled` is `true` without `--ack-auto-refund-armed`. The ordered sequence now lives in `FIN_B1B_RELEASE_PREFLIGHT.md` §4, which is the combined B1+B1b package |
| `tenants/whitecross/platform/settlementScan` | 404 | created by the first enabled sweeper pass |
| **Live Stripe endpoint `we_1TU3wwRfgDnpYJzPS6E4eUxU` event list** | Last recorded 2026-08-29: `["charge.refunded","checkout.session.completed","refund.updated"]`. **Re-verified behaviourally 2026-09-18** — the BL-4 activation record names those three and shows `stripeWebhook` handling `charge.refunded` and `refund.updated` that day | owner reads it (Dashboard, live mode) and records id + sorted list in the ledger row. **Owner decision 2026-09-21: `charge.updated` is NOT added — leave the list exactly as it is.** The cost is stated in `FIN_B1B_RELEASE_PREFLIGHT.md` §4 step 3: the fee then completes only through the indexed due pass |
| Event coverage of the B1 branch | `settlements.js:153-155`: `charge.succeeded`/`charge.updated`/`charge.captured`, `checkout.session.completed`/`async_payment_succeeded`, `payment_intent.succeeded` | With today's recorded subscription B1 already runs on `checkout.session.completed` (charges looked up by PaymentIntent). A `balance_transaction` that is `null` at that moment leaves `settlementSync` pending and the sweeper fills it on its next pass (≤ 15 min). Adding `charge.updated` makes the fee arrive with Stripe's own update. `charge.succeeded` is not needed (the session event covers the capture). ~~**Recommended: add `charge.updated` only.**~~ **Not adopted — owner decision 2026-09-21: the subscription is not widened.** The consequence is that the indexed due pass becomes the only path that completes a pending fee, which is why the index step is blocking rather than optional Whether `balance_transaction` is already set at `checkout.session.completed` in live mode is **unverified** (the test API showed it `null` on `charge.succeeded`) |
...
3. **Stripe** — owner records the live endpoint event list. ~~then updates it to the recorded list **plus**
   `charge.updated`~~ — **withdrawn 2026-09-21 (owner): the subscription is not widened.** Record the list;
   change nothing. Consequence in `FIN_B1B_RELEASE_PREFLIGHT.md` §4 step 3.
...
- New finding: no activation tool exists for `settlementLedgerEnabled`. *(Closed 2026-09-21: `scripts/activateSettlementLedger.mjs`.)*

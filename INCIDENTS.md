...
**Severity:** 🟠 High (every future appointment entered from the panel reached the customer silently; workaround = tell them verbally) · **Owner:** owner (noticed the customer never got the mail) + Claude · **Status:** ✅ Resolved, ⏸️ NOT DEPLOYED (commit `50fab67` local, `hosting:salown` awaiting owner approval) · **Affected area:** admin panel booking entry → confirmation/reschedule email gates
...
**Related:** commit `50fab67` (local) · prior half `802aaed` · policy origin `3ff3712` · files `src/components/WalkInForm.tsx`, `src/pages/Dashboard.tsx:1138`, `functions/src/emails/index.ts:96`, `functions/src/index.ts:3928` · edit-log-salown 2026-08-04 · claim `ADMIN-BOOKING-TYPE-1--alish--abt1`

...
**Regression Tests:** `functions/src/bookings/createBooking.test.js::3h–3q` (10 tests: free-barber assignment, all-taken ⇒ SLOT_CONFLICT, service qualification, passive/off-shift candidates excluded, honest policy reason vs slot reason, least-busy + order tie-break, the name-in-`barberId` quirk, explicit-pick parity, mid-transaction reassignment, reserved-id rejection). Full suites green: functions 582, frontend 507. The emulator suite could NOT be run on this machine (no Java runtime) — `tx.get(CollectionReference)` is proven by the fake's OCC + the Admin SDK contract (`CollectionReference extends Query`), not by a live emulator run.
**Related:** commits `d47e187` · claim `ANY-BARBER--alish--any-barber-assign` · files `src/pages/BookingPage.tsx`, `functions/src/bookings/createBooking.ts`

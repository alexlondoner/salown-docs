...
**Root Cause:** `STRIPE_SECRET_KEY` (CANLI slot) secret'ının **en son versiyonu (v4) bir TEST key** (`sk_test_51TB…`) idi. `createCheckoutSession` non-testMode dalında `chargeGBP` için `STRIPE_SECRET_KEY`'i kullanır (`functions/index.js:175`) → değer test olduğundan Stripe **cs_test** session üretti (gerçek kart reddedilir). Test key, muhtemelen test-mode kurulumunda (`f92ac1b8`/`017842e2`, 2026-05-07) veya sonraki bir key rotasyonunda **yanlışlıkla `STRIPE_TEST_SECRET_KEY` yerine `STRIPE_SECRET_KEY`'e** `secrets:set` ile yazılmıştı (v4). Fonksiyonlar o versiyondan sonra deploy edilmediği için **uykuda kaldı** (Elie 07-19 hâlâ cs_live aldı). **Claude'un 07-21 email-extras `functions:whitecross` deploy'u** secret'ı "latest = v4"e yeniden bağlayınca canlı checkout sessizce test'e döndü. (v2 de "mk_…" bozuk bir değerdi — slot geçmişi kirli.)

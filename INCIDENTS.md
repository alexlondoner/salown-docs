...
**Severity:** 🟠 High (the whole public self-booking funnel for whitecross was down for ~3 days — every customer hit a hard rejection at the last step; no workaround on salown.com, though whitecrossbarbers.com kept working, and no data was corrupted) · **Owner:** owner (customer-facing report) + Claude · **Status:** 🟡 Open — fix committed `c01e4b5`, isolated hotfix branch `hotfix/docid-1` built and green, **NOT deployed** (holding: a hosting deploy from `main` would co-release another session's unapproved OPT-1 work) · **Affected area:** hosted booking page → `salownCreateBooking` service resolution (`src/pages/BookingPage.tsx`, `src/pages/SalonSitePage.tsx`)

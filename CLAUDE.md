...
| [STAFF_OFFBOARD_TERMINAL.md](docs/STAFF_OFFBOARD_TERMINAL.md) | **The canonical departure (LIVE)** — why a departure is THREE authorities (`barbers.status` · `staffComp.effectiveTo` · the rota terminal archive) and what happens when only two are written (the Arda drift, INCIDENTS 2026-08-29); the `OFFBOARD` op on `salownStaffLifecycle`, the inclusive `lastWorkingDay`, the owner/super-admin split and the backdate price (reason + evidence + attribution); the ONE transaction and the two engine seams that make it one; `ROTA_OFFBOARD` and why `salownRotaTransaction` refuses it by name; **why the legacy `workingDays` mirror must NOT be re-authored**; the error codes, the operator runbook and the fact that there is no undo |
...
| [STAFF_AVAIL_GAP_PLAN.md](docs/STAFF_AVAIL_GAP_PLAN.md) | DECISION + PLAN, not started (`STAFF-AVAIL-GAP`, `CONFIRMED_OPEN`): New Booking + Reschedule have no server-side passive/conflict/hours enforcement (raw Firestore writes, name-keyed `barberId`); Walk-in checks passive but not conflict; Block Time is the one race-safe, fully-gated surface. Identity migration is a non-issue (`resolveBarberRef`/`matchesBarber` already resolve legacy name-keyed records at read time). Server infra for New Booking already exists unwired (`createBookingCore`'s `surface:'staffApp'`). The one real open product decision is conflict policy (reject-by-default vs. today's overridable pattern) — tied to the squeeze-in feature, must not regress it |

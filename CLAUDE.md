...
| [STAFF_AVAIL_GAP_PLAN.md](docs/STAFF_AVAIL_GAP_PLAN.md) | **D1-D7 DECIDED 2026-09-12** (`STAFF-AVAIL-GAP`), code/deploy NOT started: server enforcement approved for New Booking+Reschedule; passive HARD everywhere no override; conflict+hours reject-by-default with `owner`-only logged override (not `admin`, no new role); leave applies to New Booking/Walk-in/Reschedule only (Block Time excluded); mola/`BLOCKED` conflicts refuse outright, no override path at all; race-safety requires per-pair cross-flow proof, not "uses the blocks.ts pattern". Phase 0 (design/verification only, no source changed) is approved; Phase 1+ needs a separate approval |

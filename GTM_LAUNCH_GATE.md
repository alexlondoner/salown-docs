...
- **2026-09-11 · A5 `T-e` 3+4 — CLOSED.** Path 3: owner role round-trip observed in both the staff doc
  and the Auth claim after the flag-flip release (`R-2026-09-11-A`). Path 4: **owner decision — remove,
  do not redesign.** `registerMeAsAdmin` (handler, button, the state and prop only it used) removed in
  salown-app `819b0a9`, released `hosting:salown` `8a2c104476128bf3` → `c389253b23bb981b`
  (`R-2026-09-11-B`); the served Settings chunk no longer carries it, the owner's Staff tab re-checked
  live. The super-admin keeps managing salons through the super-admin authority + tenant selector and
  is never given a tenant owner record or claim from Settings. No owner repair was performed. The
  proof line above ("read-only Auth listing … the old path's false 'Saved' is gone") is met for path 3
  by the 2026-09-11 live test and for path 4 by removal.

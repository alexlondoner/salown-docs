...
- ✅ **Ürün bilgisi / sitemap CANLI (2026-07-14, `58668af` [lokal, push başka session'ın onayına bağlı] + `functions:salown:askAI` deploy):** `functions/src/ai/productGuide.ts` — kullanıcıya-dönük sitemap (panel + staff app + müşteri tarafı) + ~18 nasıl-yapılır + asistan notları (delete=owner-only, plan-gate). Prompt yönlendirme: how-to→SADECE rehberden (UI uydurmak yasak, bilinmeyende support'a yönlendir), veri→snapshot, cevap kullanıcının dilinde. ⚠️ Kaynak RAW ROADMAP/CLAUDE.md DEĞİL (iç doküman sızıntı riski) — curated rehber. **Bakım kuralı: user-visible feature çıkınca productGuide.ts'e satır ekle + askAI hedefli deploy.** KALAN: owner okuma turu (rehber içeriği doğrulaması) + feature-flag farkındalığı ve tool-use → C4.

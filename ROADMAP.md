...
**G1 — Minor** · 🔵
- **Confirmation email iz bırakmıyor (observability)** (2026-07-13, yanlış-alarm dersi): `functions/src/emails/index.ts` confirmation sender booking doc'una FLAG YAZMIYOR (yalnız console log) → panel/veriden "email gitti mi?" görülemiyor, owner bugün alarm verdi (loyalty `loyaltyEmailSent` yazıyor, tutarsız). Fix: send başarısında `confirmationEmailSentAt` yaz (loyalty paritesi); reschedule/cancel sender'ları da aynı. ⚠️ no-op write trigger tetiklemez notu ilgisiz (ayrı alan).

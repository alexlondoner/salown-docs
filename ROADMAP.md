...
| `hosting:whitecrossbarbers-saas` — whitecrossbarbers.com | version **`e6be08684d312ce7`** · release `1786401587236000` | 2026-08-10T22:39:47.236Z | LIVE, source **UNKNOWN/HYBRID** — `WCP-1`. ⛔ An authorised passive-authority release was **stopped before deployment** 2026-08-13 (`R-2026-08-13-X`); `WCP-5` |
...
| P0 | `WCP-5` | Whitecross premium site still resurrects a **departed** barber from a stale `shiftChanges` override — fix written and gated, **not live** | `PUSHED_NOT_LIVE` / `BLOCKED` | whitecross-site | released | `REL-4` / `WCP-1` | `8c655389` (impl) · `5202cad` (checkpoint) | **absent** from `e6be08684d312ce7` — served `script.js` still reads `shiftChanges` before lifecycle in both paths | Cannot ship alone: public root is `.`, so a deploy also activates the held W1/C1 cutover and `bc25d257` (which blanks the live `Double Points` banner). Needs `REL-4` | 2026-08-13 |
| P1 | `PASSIVE-R3` | `passive` raised to an ABSOLUTE stop, resolved **before** the dated `shiftChanges` override, on every salOWN display surface | **`LIVE_VERIFIED`** | salown-app | released | — | `78124db` / `00cfc43` | `hosting:salown` **`84eb7dda5e1b2140`** (first shipped alone in `2eff0455ed404c15`, release `1786640876872000`) | Whitecross half is `WCP-5` and stays open | 2026-08-13 |
...
| P1 | `SPLIT-B-JACK` | The one live booking already written in the malformed shape (`splitAmount "15"`, `splitSecond ""`, no `paymentAllocation`) | `PLANNED` — **deliberately unrepaired** | data | — | `SPLIT-B` (done) | — | `tenants/whitecross/bookings/3ori9n79QSj09Xyu96fQ`, `updateTime` `2026-08-13T13:12:09.460Z`, byte-unchanged | Guarded owner-authorised repair, its own pass. Receipt/loyalty are **correct** and must not move: £49.80 total, 64 pts = £3.20 | 2026-08-13 |
...
| `hosting:whitecrossbarbers-saas` | whitecross-site | **UNKNOWN / HYBRID** | `e6be08684d312ce7` · rel `1786401587236000` · 2026-08-10T22:39:47Z | **`STATUS_UNKNOWN`** | 2026-08-13 re-measured: served `script.js` sha256 `ffa63589…e77637`, 123,185 bytes, matches **no commit** in a full `git log --all` scan. Hours fix present; `salownCreateBooking` **0** (7 in `main`), `expectedPaymentFlow` **0** (1 in `main`), `isBarberPassive` **0** (3 in `main`); served `index.html` still carries `Double Points` + `2× loyalty points` and no `doublePointsMultiplier` ⇒ `bc25d257` **absent** | `REL-4`, then `WCP-2` + `WCP-3` + `WCP-5` together |

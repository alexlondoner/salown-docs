...
- 🔵 **Ürün bilgisi / sitemap (sıradaki):** AI'a salOWN'un yapısını öğret — hangi sayfa ne yapar, temel nasıl-yapılır'lar (walk-in alma, kampanya gönderme, discount code, working hours, loyalty) → owner "bunu nerede yaparım?" sorularına Gemini-Workspace tarzı cevap alır. ⚠️ **Kaynak RAW ROADMAP/CLAUDE.md OLAMAZ** — onlar iç mühendislik dokümanı (güvenlik açıkları, commit'ler, altyapı detayı; her tenant owner'ına sızdırılamaz). Doğrusu: **curated, kullanıcıya-dönük ürün rehberi** (statik metin, functions'ta sistem prompt'a eklenir; feature-flag'e göre bölümler — örn. parser'sız tenant'a parser anlatma). İleride C4 tool-use ile birleşir (sunucu-tarafı aggregation = aritmetik hatası sınıfı kökten ölür + tenant-scoped guardrail).

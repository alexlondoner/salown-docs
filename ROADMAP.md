...
| P0 | `FIN-PL-SCOPE-SMOKE` | The 2026-08-14 Finance/P&L release has **no screen verification** — ten authenticated read-only checks outstanding | `PLANNED` | salown-app | — | an authenticated **Whitecross** session | — | live `6cc0254d73227a96`, byte-proven only | Run the ten checks: All-view parity · Cash/Card additivity · Service/Add-ons "not derivable per tender" · whole-transaction scopes · method-specific Tips · waterfall reconciliation · Daily P&L labels · Overview reset · return-to-Daily state · console clean. Then promote `FIN-PL-SCOPE` and `FIN-TENDER-SCOPE-P1` to `LIVE_VERIFIED` | 2026-08-14 |
| P0 | `FIN-PL-SCOPE` | Profit-and-loss surfaces were tender-filtered — a P&L charged ONE method's revenue with EVERY method's costs | **`LIVE_UNVERIFIED`** — artefact live & byte-proven, ⛔ behaviour NOT verified in production | salown-app | released | ADR-024 | **`29a7016`** · `5bd2b8d` · `b34d984` | `hosting:salown` **`6cc0254d73227a96`** · release `1786699000997000` · 2026-08-14T09:16:40.997Z | Whole waterfall + Daily Ledger P&L columns read the authoritative roll-up (0 filtered reads, 0 filter branches inside the card, source-asserted); filter can be non-All only where its control renders. Owner confirmed the **shell** loads on Whitecross; the ten Finance/Reports screen checks are **outstanding** — controlled browser was on tenant salOWN and authenticating was declined. Ledger `R-2026-08-14-A` | 2026-08-14 |
| P1 | `FIN-TENDER-SCOPE-P1` / `P1.1` | `productRev` whole-transaction/non-additive; `Service = Gross − Product` withheld under a filter; the nine Breakdown measures scoped **individually** across three scopes (Tips is leg-additive, Service/Add-ons withhold, four are transaction) | **`LIVE_UNVERIFIED`** — artefact live & byte-proven, ⛔ behaviour NOT verified in production | salown-app | released | `SPLIT-B2` | `0fe662a` · `6f4d335` · `6148dd7` | `hosting:salown` **`6cc0254d73227a96`** | Shipped in the same release as `FIN-PL-SCOPE`. No pro-rata allocation invented anywhere; `platformFees` deliberately untouched — it is one of five whole-period costs, and ADR-024 rather than a per-tender scope is its answer. Ledger `R-2026-08-14-A` | 2026-08-14 |

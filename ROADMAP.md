...
**A2 — Stripe Connect (ödeme)** · 🟡 **Faz 0+1 backend ✅ DEPLOYED (2026-07-04, TEST mode) · UI + policy/refund kaldı**
...
- 🔴 **UI kaldı:** Settings→Integrations "Connect with Stripe" butonu + rozet + Disconnect + `paymentMode`/`defaultDepositAmount` alanları — `src/`'de HİÇ referans yok, henüz yazılmadı (başka session yapacaktı, olmamış). Kontrat: Start→url'ye git, dönüşte Status→rozet.

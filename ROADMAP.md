...
**H4 — Parser mail onboarding: app-password → parse-inbox** · 🔵 **KARAR BEKLİYOR (2026-07-02)** · 🔴 onboarding-kritik
**Sorun:** `salownParseEmails` her tenant'ın Gmail'ine **app-password + IMAP** ile bağlanıyor (`functions/index.js:3065`). Salon sahibi 2FA aç → App passwords → 16-hane yapıştır yapmak zorunda = **non-teknik için onboarding katili** + düz-metin şifre (= **T-b** güvenlik borcu) + Gmail'e kilitli + Google app-password'leri kısıtlıyor.
**Kilit içgörü:** Parser'ın salonun inbox'ına değil, **Booksy/Fresha/Treatwell'in gönderdiği bildirim maillerine** ihtiyacı var → o mailleri Salown'un kendi adresine yönlendirtirsek tenant kimlik bilgisi HİÇ tutmayız.
**Seçenekler:** (A önerilen) **Salown parse-inbox** — `{tenant}@parse.salown.com`, salon bildirim mailini oraya çevirir/forward eder; inbound-parse webhook (Cloudflare Email Routing/SendGrid) → Cloud Function → gerçek-zamanlı parse. App-password/2FA/OAuth YOK; **T-b buharlaşır**; Gmail'e bağımsız. Bedel: mail-alan domain kurulumu. · (B) Gmail OAuth read-only — tek tık ama Google restricted-scope (CASA) denetimi pahalı/yavaş. · (C) app-password + rehberli sihirbaz — yara bandı, kök sorun kalır.
**Not:** A seçilirse **T-b (yukarıda) gereksizleşir** (şifre saklamıyoruz). Mevcut whitecross app-password IMAP geçiş sırasında çalışmaya devam eder. Karar verilince ADR ([DECISIONS.md](DECISIONS.md)) + faz planı yazılacak.

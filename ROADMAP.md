...
**H4 — Parser mail girişi: parse-inbox hybrid + token izolasyon** · ✅ **PİLOT TAM CANLI (2026-07-13/14)** — forwarding kuruldu + gerçek maille tam yaşam-döngüsü tatbikatı GEÇTİ (create/reschedule/chain/cancel × iki boru, sıfır çift kayıt; detay aşağıda). ✅ **İLK ORGANİK MÜŞTERİ MAİLİ GÖZLENDİ (2026-07-14 07:28Z):** "Rory Breach: new booking Tue 14 July 09:00" boru üzerinden uçtan uca aktı → `PARSED`, dedup `skipped=1`, sıfır çift kayıt. Yarışı bu kez IMAP kazandı (import onda; boru dedup'la tanıdı) — iki boru paralel koştukça yarış sonucu değişken, önemli olan kanıt: **organik mailde routing+parse+dedup üçü de doğru çalıştı.** Boru'nun `imported=1` ile kazanması IMAP emekliye ayrılınca kendiliğinden norm olur. Kalan: Fresha/Treatwell forward filtreleri (owner, opsiyonel) + herohairs geçişi + model oturunca IMAP emekliliği (T-b buharlaşır). ⚠️ Ev işi (2026-07-14, log ile YENİDEN doğrulandı — hâlâ sürüyor, son gözlem 09:38Z): tatbikatın 3 test maili whitecross Gmail'de UNSEEN kalmış → IMAP cron 5 dk'da bir aynı "Booksy cancel/reschedule not found" üçlüsünü yeniden işleyip logluyor (zararsız — yazma yok — ama gürültü + gerçek hatayı maskeler); çözüm: owner o 3 maili okundu işaretlesin YA DA parser'a terminal not-found'da mark-seen kuralı (dikkat: out-of-order retry davranışını bozmadan)

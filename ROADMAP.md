...
- **🔴 Kalan delik 1:** ürün satışı (`firestoreActions.ts:368`) ve blok (`:411`) sadece `barberId` yazıyor, `barberName` YOK → silinirse o satışların ismi kaybolur. **Fix:** iki yazıma da `barberName` ekle (ileriye dönük, risksiz, wage mantığına dokunmaz).

...
- ⏸ **Go LIVE (gerçek para)** — Stripe live keys + owner kararı; komisyon aktivasyonu (`application_fee` %0 kablolu); success'e refund testi. **Waiting.**
- 🔵 **Premium custom-site deposit toggle** — premium tenant'ın kendi sitesi (whitecross-site tipi) deposit on/off+tutarı hardcode yerine panelden okusun. Köprü hazır (`public/booking` projeksiyonu `2db8721` paymentMode/defaultDepositAmount taşıyor). Spec: [STRIPE_CONNECT_PLAN.md](STRIPE_CONNECT_PLAN.md) §G. ⚠️ canlı-gelir-yolu riski.

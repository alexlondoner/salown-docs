...
- 🔵 **Sonraki cleanup slice — orphan HELPER FONKSİYONLARI (2026-07-06 tespit):** slice 2/3a/3b orphan *memo*'ları temizledi ama `Marketing.jsx` tepesindeki 5 *helper fonksiyon* öksüz kaldı — `getBookingDurMins`, `shopOpenHoursPerWeek`, `barberWorksOn`, `getSlotType`, `cellStyle` (her biri 1 geçiş = sadece tanım, 0 çağrı). Occupancy'ye taşınırken `OccupancyPanel.jsx`'e **kopyalanmışlar** (orada 20 geçişle canlı kullanılıyor), Marketing'deki orijinaller silinmemiş. **Güvenle silinebilir** — build 0-error, canlı memo'lar (filteredAll/weeklyTrend/customerInsights/overview/campaignAudience) bunları kullanmıyor. ✅ **Occupancy sağlamlığı doğrulandı:** `OccupancyPanel.jsx` 628 satır, tam heatmap motoru, Reports `occupancy` tab'ında `<OccupancyPanel tenantId/>` çalışıyor — bozulma YOK.

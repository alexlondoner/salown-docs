...
| P0 | `FIN-PL-SCOPE` | The Overview **P&L Statement** is silently tender-filtered — the control renders only on Daily/Monthly but the state is page-global, so a P&L charges ONE method's revenue with EVERY method's costs | `PLANNED` | salown-app | — | — | — | live 2026-08-13: All **+£51.60** · Cash **−£162.00** · Card **−£6.40** — off by exactly £220.00, the whole cost base twice | Adopt **ADR-024**: Overview P&L is always whole-period `All`; entering Overview explicitly neutralises the filter; cash/card is collection analysis, not profitability. This is also **why `platformFees` was deliberately not scoped** in `FIN-TENDER-SCOPE-P1` | 2026-08-13 |
| P1 | `FIN-TENDER-SCOPE-P1` | `productRev` whole-transaction/non-additive; `Service = Gross − Product` withheld under a filter; Reports/Breakdown measures scoped **individually** (Tips is leg-additive, the other six are not) | `PUSHED_NOT_LIVE` | salown-app | released | `SPLIT-B2` | `0fe662a` · `6f4d335` | live is **`422bcb40aab7df89`** (`SPLIT-B2`) — this work is **not** in it | Owner release window. No pro-rata allocation invented anywhere; `platformFees` deliberately untouched pending `FIN-PL-SCOPE` | 2026-08-13 |

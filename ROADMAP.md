...
**S2 — Ödeme modeli / compensation (staff başına)** · 🟡 **FAZ B UI KODLANDI (2026-07-15) — LOCAL, push/deploy owner onayı bekliyor**
> **2026-07-15:** Staff Hub Faz B salown-app'te 8 local commit (`686ae45..9332d15`, PUSH EDİLMEDİ — owner önce localde görecek). Kapsam: compUtils+staffCompActions (append-only dönem matematiği, unit-testli) · PayModelChip (owner-only) + Former staff bölümü · sekmeli drawer (Profile/Availability/Pay/History) · 3 comp özet kartı + 3-adım CompChangeFlow (effectiveFrom min=bugün) · passive=comp-dönem-kapat bağlantısı · rules staffComp bloğu + **catch-all READ daraltması** (OR-semantiği gereği şart; test 95/95, 12 yeni S1 vakası). Balance satırı Faz C'ye işaret eder ("Tracked in Finance"). **Deploy sırası: hosting önce → rules EN SON (canlı çek+diff).** M1 göç (partnerConfig→staffComp dry-run CSV) henüz koşulmadı. §7 Faz B "emniyet fix'leri" (occupancy resolver, legacy active-okuyucular, Reports arşiv=delik 2) bilinçli olarak AYRI mini-koşuya bırakıldı (Keep Scope Narrow). Not: `whitecrossbarbers@`=tenantRole:staff → Pay sekmesi görmez (doğru); owner testi `aerulas@` ile.

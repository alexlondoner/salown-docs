...
**A2 — Stripe Connect (ödeme)** · 🟡 **Faz 0+1 backend YAZILDI (deploy bekliyor, `863e3db`) · policy/refund kaldı**
...
- 🟡 **Faz 1 — Checkout (backend yazıldı `863e3db`, deploy edilmedi):** `salownCreateCheckoutSession` (public callable, tutar SUNUCUDA servis doc'undan, Direct charge `{stripeAccount}`, deposit/full per `paymentMode`, metadata=docId/tenantId/paymentType) + **`salownConnectWebhook`** (YENİ paralel webhook — tek platform key + `STRIPE_CONNECT_WEBHOOK_SECRET`; tenant'ı metadata'dan çözer, `event.account` vs stored `stripeAccountId` cross-check; `paidAmount`/`remaining`/`paymentState` yazar). Eski `salownStripeWebhook` DOKUNULMADI. BookingPage checkout-session'ı dener, başarısızsa static link'e düşer (stripe kapalıyken inert). **Deploy tonight:** hedefli → `firebase deploy --only functions:salownConnectStart,functions:salownConnectCallback,functions:salownConnectDisconnect,functions:salownConnectStatus,functions:salownCreateCheckoutSession,functions:salownConnectWebhook` (salownInboundEmail'i DAHİL ETME → ayrı secret ister). Secret'lar: `STRIPE_SECRET_KEY`+`STRIPE_CONNECT_CLIENT_ID`+`STRIPE_CONNECT_WEBHOOK_SECRET`.

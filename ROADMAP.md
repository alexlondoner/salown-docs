...
- 🔵 **Slice 3b KALAN (owner ile):** (1) **Revenue SSOT** — OverviewPanel gross `bookingRev` vs Reports net/paidAmount aynı sayfada görünür oldu, tek kaynağa indir (Finance ile tut). (2) **Client-identity SSOT** — Reports `clientMap` (basit phone||email||name key) vs `buildAudience` (union-find çapraz-kaynak) hâlâ farklı sayıyor (Unique 293 vs ~280, Returning 70 vs 82); tek kimlik kaynağına indir. (3) **Tasarım polish** — owner tercihi: iki-kolon "sayılı grafik sol / cash-card sağ"; rakamlar+% artışlar daha büyük/belirgin, boşluk azalt.

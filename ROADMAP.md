...
| P1 | `SEC-CATCHALL-1` | The platform-wide super-admin catch-all `match /{document=**} { allow read, write: if isSuperAdmin(); }` ORs across every match, so NO per-collection rule can constrain a super-admin browser session | `PLANNED` | salown-app | — | — | — | live ruleset `640c3dae-a9c8-4cb3-80c4-bc189e72874a` | Surfaced twice now as disclosed residue — by `scripts/testPromotionSnapshotRules.py` (a forged `loyaltyPromotionSnapshot`) and by `STAFF-START-A1` (a pre-start booking). Surfaced a THIRD time by **`FIN-DATED-ROTA-R2b`** (2026-08-16), which is the first package it actually BLOCKS: R2b's brief required canonical rota writes to be denied for every role *including* a browser super-admin, and that half is **not achieved**. R2b also supplies the missing ATTRIBUTION proof — with the root grant's `write` removed and nothing else changed, all three R2b clauses hold (`test/rules/staffRota.emulator.test.js`, "the super-admin bypass is the ROOT catch-all, not an R2b clause"), so the residue is entirely the catch-all's and scoping it is the whole fix. No package could close it: the grant is above them, and no clause inside a collection can take it away. It is **asserted rather than described** by the last case in `test/rules/availabilityFrom.emulator.test.js` and by two cases in the staffRota suite plus the `RESIDUE` lines of `scripts/testStaffRotaRules.py`, so scoping it will turn those tests red and force this row to be revisited. **Next:** decide between removing the catch-all and field-scoping it (e.g. super-admin write limited to the collections the super-admin console actually writes), then re-run every rules suite — this is a platform-wide grant with real blast radius, so it is its own package with its own live-parity proof, never a side edit | 2026-08-16 |

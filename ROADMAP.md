...
| P0 | `FIN-DATED-ROTA` | Effective-dated rota — a rota is a HISTORY, not a setting, so a rota edit stops re-pricing closed months | `IN_PROGRESS` — R1/R1b (fold) + R2 (engine) + R2b (boundary + rules) are **PUSHED_NOT_LIVE**; nothing is deployed and nothing is wired | salown-app | — | `FIN-PERIOD-CLOSE` | `57b1ee1` (R2) · `1c42976` (R2-EV.1) · **`e66a877`** (R2b) | — | **URGENT.** Root cause of the wage-integrity incident. The 2026-08-13 restore was correct **and** re-priced every closed month a second time — proof the mechanism is still armed. Sub-items below. **Next: `FIN-DATED-ROTA-R2c`** (the UI cutover), which is a hard prerequisite for deploying R2b's rules | 2026-08-16 |
...
| P0 | `FIN-DATED-ROTA-R2b` | The server-authoritative boundary in front of the engine, plus canonical storage ownership in `firestore.rules` | **`PUSHED_NOT_LIVE`** — callable and rules prepared, **not deployed** | salown-app | released | `FIN-DATED-ROTA-R2` | **`e66a877`** | **not deployed** — rules ruleset unchanged, no Function deployed, export absent from the live surface | One callable, `salownRotaTransaction`. Authority is the tenant's EXISTING staff-management policy, audited out of `setStaffRoleCore` + the `OWNER_ONLY` nav gate and PINNED: **owner|admin + super-admin break-glass; ordinary `staff` DENIED**. Identity, tenant, role, origin, revision, hashes, cacheState and timestamps are all server-derived and refused by name in the body. Authorization is re-read **inside** the write transaction via a decorated Firestore handle, so an offboarding committed mid-flight stops the append with zero writes. Rules: `staffRota/**` client-unwritable in every role; `workingDays`/`dayHours`/`hours` unwritable on barber UPDATE with the guard **hoisted above** the role disjunction; CREATE deliberately exempt (documented bootstrap seam). Archive: R2-EV.1's four exclusions removed, 148→154 files, digest `b92ebf31…`, identical twice from a pinned clean worktree; exports 73→74, the diff is exactly `salownRotaTransaction`; a PERMANENT negative control fails the build if the callable's runtime module is ever re-excluded. ⚠️ **ONE BLOCKER LEFT ON DEPLOY** (was two). ✅ **`SEC-CATCHALL-1` CLEARED 2026-08-16 (`750c99f`, pushed not deployed)** — the root catch-all's write grant is removed, so R2b's "no browser client may write either, in any role" is now literally true, super-admin included; the R2b suite's two RESIDUE tests were inverted into gates (§5a denies, MUTATION 4 re-opens on restoring the grant) and `super-admin` joined `CLIENT_ROLES`, so R2b's own matrix now covers it. ⛔ **STILL BLOCKING: `FIN-DATED-ROTA-R2c`** — `Barbers.tsx` sends all three cache fields on every save and re-stamps `dayHours` provenance, so deploying these rules first breaks the Team Members editor. **Next:** R2c (UI cutover), then rules deploy LAST | 2026-08-16 |
| P0 | `FIN-DATED-ROTA-R2c` | Team Members canonical rota cutover — stop every browser surface writing `barbers.workingDays`/`dayHours`/`hours` directly, and close the new-team-member bootstrap seam | **`BLOCKED`** — scoped and designed, **nothing implemented, nothing pushed, nothing deployed** | salown-app + whitecross-site | — | ~~`SEC-CATCHALL-1`~~ (✅ landed `750c99f`) · `STAFF-START-A2` · the §3 propagation decision | — | — | Design + complete writer inventory: **`docs/FIN_DATED_ROTA_R2C_DESIGN.md`**. **Blocked twice over.** (1) ✅ **RESOLVED 2026-08-16** — the prerequisite `SEC-CATCHALL-1` landed at **`750c99f`** (`PUSHED_NOT_LIVE`, no deploy) and `alish/sec-catchall` released its claim, so `firestore.rules`, `SYNC.md` and the rules suites are FREE again. The historical note follows: it was being written by that session, whose claim LOCKED those paths. Claim rule 7 is a hard stop and this pass yielded on every one of those paths. (2) The brief's own §3 stop condition is MET: **there is no server-authoritative team-member provisioning flow to extend** — `createStaffUser` provisions login identity only, `provisionTenant`/`approveApplication` create the *owner's* barber doc only, and an ordinary member's `barbers/{id}` is created solely by the browser at `Barbers.tsx:451`. **Three structural gaps, all recorded rather than hidden:** *Gap 1* — the engine returns `BARBER_NOT_FOUND` (`rotaWriter.ts:1062`) **61 lines before** the `attachExtraWrite` co-commit hook (`:1123`), so a provisioning caller can never create the subject. The transaction shape itself is sound and this is now **measured, not assumed**: on the repo-pinned emulator, `tx.get`(absent) → `tx.create` → `tx.update` on one ref **COMMITS** (4/4 probes; read-after-write correctly throws). Smallest fix = one server-only optional `provisioningSubject` dep honoured for `ROTA_START` on an absent subject, plus one `salownProvisionTeamMember` callable whose `attachExtraWrite` creates profile fields only while the engine publishes the cache. *Gap 2* — `Settings.tsx:545` salon-hours propagation is inherently **multi-subject** and the callable is single-subject, so N calls, no spanning transaction, silent partial salon state; needs an owner decision between a salon-scoped period, a fan-out callable, or withdrawing automatic propagation. *Gap 3* — **no `staffRota` header exists anywhere**, so 100% of barbers are legacy on day one and every first edit is a `ROTA_START`, which cannot be backdated — so `STAFF-START-A2` (herohairs' sole barber has no `availabilityFrom`) is a de-facto prerequisite. **New finding not in R2b's record:** `whitecross-site/barber-panel/src/pages/Barbers.js:131` writes all three fields and is configured to **two live hosting targets** (`whitecrossbarbers-admin`, `whitecrossbarbers-owner`), so it is on the rules-deploy blast radius in a third repo. **Also:** `provisionTenant`/`approveApplication` bypass rules and keep minting cache-without-log records — closing the browser seam alone does not close the bootstrap seam. **Next:** ~~land `SEC-CATCHALL-1`~~ ✅, close `STAFF-START-A2`, take the §3 decision, then implement in the §5 release order (functions → hosting → whitecross-site → **rules LAST**) | 2026-08-16 |

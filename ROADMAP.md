...
**C9 — Client kartı premium redesign + puan-harcama görünürlüğü** · 🔵 Planlandı (2026-07-11 · owner istek)
Harcanan puan verisi VAR ama görünmüyor: `loyaltyRedeemedValue` checkout'ta yazılıyor (`CheckoutPanel.tsx:930`) ama Clients hiç okumuyor; `totalDiscount` hesaplanıyor (`Clients.tsx:187`) ama gösterilmiyor; loyalty timeline sadece manuel ayarları listeliyor (checkout kazanım/harcama yok). **Faz 1 (ucuz, 0 yeni read):** Loyalty sekmesine lifetime şeridi (Points used £ / Discounts £ / Total value) + birleşik timeline + stats'a "£ saved". **Faz 2 (tasarım):** kart full-height premium drawer, hero header + inline edit (owner: "çok küçük görünüyor, edit çok basit açılıyor"). Tam spec + kod çapaları: [CLIENT_CARD_V2.md](CLIENT_CARD_V2.md).
- **Gate:** TS migration feature-freeze — kod 2026-07-14'ten önce yazılmaz (12=rc3, 13=ürün-doğrulama). Faz 1 migration-sonrası ilk uygun dilim adayı.

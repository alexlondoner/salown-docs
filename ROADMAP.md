...
4. ✅ **C5-B — bulk yol da damgalıyor (2026-07-14, `64c2698` + functions deploy):** C5/C5-A yalnız TEKLİ gönderimi (`sendMarketingEmail`) kapsıyordu; Campaigns redesign'ın bulk yolu (`sendCampaignBulk`) hiç damga yazmıyordu → whitecross lapsed30'a aynı "We miss you" blast'i 9 günde 3 kez gitti (04/06/13 Temmuz, 76/78/75 kişi). Artık bulk'ta her başarılı alıcıya `lastMarketingSentAt` (+lapsed/re-engage'te `reengagementSentAt` — BulkCampaignPanel lapsed segmentlerde `campaignType:'re-engagement'` yollar —, birthday'de `birthdayCampaignYear`) + `campaignsSent` log; doc'suz re-engage alıcısına C5-A find-or-create (TEK clients snapshot'a karşı eşleşir, per-alıcı scan yok). "Slipping away" kart sayacı compose ile aynı 30g suppress'i uyguluyor; insights "Send offer" geçersiz `'lapsed'` segment key'i (inSegment default'u → HERKESİ seçiyordu) → `'lapsed30'`. 13 Temmuz koşusunun 75 alıcısı geriye dönük damgalandı (script, 75/75 isim→doc). Residual: `all`/`haspoints` listelerinde "son N günde gönderilen" gizleme yok (veri hazır: `lastMarketingSentMs`). Detay: INCIDENTS 2026-07-06 KALAN #2.

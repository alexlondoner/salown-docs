...
- 🔄 **P1-RECEIPT-MATH canonical receipt snapshot** — **writer half ✅ LIVE 2026-07-30** (`aeed3cf`+`5dcd5a4`): checkout persists a versioned canonical `receipt*` snapshot (`receiptMathVersion` + `I3_COLUMN_RECONCILES` confirmed present in the live salown.com bundle). **Reader half NOT STARTED** — `functions/src/index.ts:726-740` still recomputes receipt figures itself, so the loyalty receipt email can still double-count add-ons until it reads the snapshot. Open financial follow-ups tracked in INCIDENTS 2026-07-30 (staff checkout drops `soldProducts`/`soldAddOns`; admin walk-in add-on overcharge).
...
- 🔄 **LC1 landing live chat (bot-first + human handoff)** — BUILT, locally verified end-to-end, **NOT DEPLOYED, NOT COMMITTED** (owner said write-only, 2026-07-28). Bot (Haiku 4.5) answers strangers on salown.com instantly from `landingGuide.ts`+`productGuide.ts`; the founder takes over from super-admin → **Live Chat** and the bot goes silent (`mode:'human'`), "Hand back to assistant" returns it. Built not bought ([ADR-017](DECISIONS.md#adr-017--landing-live-chat-built-not-bought-bot-first)). Pieces: `hosting/chat-widget.js` (vanilla, one script tag, no Firebase SDK) · `functions/src/ai/landingChat.ts` (`salownLandingChat`, public HTTP) · `super-admin/src/pages/LiveChat.jsx`. **Zero firestore.rules change** — everything under `superAdmin/liveChat/**`, already super-admin-only; the visitor's browser never touches Firestore, it polls the endpoint (and only once a human is involved). ⚠️ Public+unauthenticated by nature = the askAI cost hole cannot be closed with auth here, so it is bounded by 4 guards (1000 chars · 40/session · 60/IP/hour · 1500/day cap + `enabled:false` kill switch, all live-tested). Full doc: [LIVE_CHAT.md](LIVE_CHAT.md). **Next:** owner approval → targeted deploy `functions:salown:salownLandingChat` + hosting + super-admin. **Maintenance rule (same family as C10):** landing copy/pricing changes ⇒ `landingGuide.ts` changes in the SAME commit.

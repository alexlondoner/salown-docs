...
> **Son revizyon: 2026-07-02** (yeniden yapılandırıldı 07-01; C2/C2b/C2c/C5 + H teması eklendi 07-02; **G1-G4 gerçek deploy durumuna düzeltildi + SSOT protokolü eklendi 07-02**; **I teması (Güvenilirlik & Teknik Borç) + Tier 2 read:true yüzeyi eklendi 07-02, kaynak [ARCHITECTURE_REVIEW_2026-07-02.md](ARCHITECTURE_REVIEW_2026-07-02.md); H1 early-access intake ✅ CANLI `a2689f9` 07-02; H2 vetted-onboarding (demo funnel + Applications approve→provision) ✅ CANLI 07-02, INCIDENTS 2026-07-02 (claim clobber guard) eklendi**; **I2 `index.js` split planı zenginleştirildi + GATE=sonraki feature'dan ÖNCE, blocker=index.js commit'siz, 07-04**; **C7 "Kur → Ölç → Öğren" otomasyon metrikleri eklendi 07-10 [dış UX review, RC3-sonrası gate]**; **parser durum-doğrulaması 07-14: I1 canary rozeti ✅ CANLI'ya düzeltildi [bayat "deploy edilmedi"], H4 ilk organik mail gözlemi ✅, bounce-checker hâlâ kırık teyit**; **YENİ tema S — Staff Management & Compensation eklendi 07-14 [owner yönü: comp modelleri wage/commission/self-employed ayrı modül + veri-emniyeti/snapshot; A'dan sonra, I2+Gate altında]**).
...
### 🎯 Sıradaki oturum — ÖNCELİK (güncellendi 2026-07-14)
> **Sıradaki: I2 Faz 2 — fonksiyon export'ları domain modüllerine (owner seçimi 2026-07-14).** Gerekçe: I2'nin kendi gate'i ("sonraki feature'dan ÖNCE") artık aktif — TS migration v1.0.0 ile kapandı, bloker yok. **Envanter düzeltmesi (2026-07-14 tespiti):** `index.js` YOK artık — migration'da `src/index.ts` oldu; **Faz 1 (helpers→modüller) fiilen TAMAMLANMIŞ** (parsers/notifications/emails/marketing/tenants/inbound/checkout/clients/finance/utils modülleri canlı + parity testli, index.ts import ediyor). Kalan = Faz 2. ✅ Dilim 1 (askAI + auth guard) CANLI 2026-07-14 — **sıradaki: dilim 2 = parsers export'ları** (`salownParseEmails`/`salownInboundEmail`/`salownParseInboxDispatch`/`salownManualImport`/`salownIcalFeed` → domain modüllerine; canlı borunun en hassası, karakterizasyon testleri `parsers/messages.test.js` yanında); sonra notifications/marketing; **stripe/bookings EN SON**. Altın kural aynen: export adı + config birebir, saf taşıma, dilim başına tek commit + hedefli deploy. ⚠️ Başlamadan: index.ts working-tree temiz olmalı + diğer session'lar functions'a dokunmamalı. (2026-07-14 gece: bekleyen push kalmadı — `58668af` dahil hepsi origin/main'de; dilim 2 session'ı yine de standart `git status` + `git log origin/main..HEAD` kontrolüyle başlasın.)
> **Arka planda açık:** H4 kalanları (herohairs parse-inbox geçişi + UNSEEN-mail log gürültüsü + IMAP emekliliği → T-b buharlaşır) · Tier 2 🔴-1 read:true yüzeyi (tenant hacmi artmadan) · G1 panel in-app notification (müşteri reschedule/cancel zili) · **YENİ tema S — Staff Management & Compensation** (owner yönü 2026-07-14: comp modelleri wage/commission/self-employed + veri-emniyeti/snapshot; I2+Gate'in ALTINDA, S1 küçük emniyet fix'i hazıra yakın).
>
> *(Önceki öncelik blokları arşive: Salı 14 Tem fix sırası ✅ TAMAMEN bitti [v1.0.0 + G5.1 + G1 üçlü + confirm-guard'lar + G6/D3/D4]; H4 pilot ✅ TAM CANLI [ilk organik mail dahil, detay H4]; `dailyFirestoreBackup` ✅ DÜZELDİ 07-13 [G1 + INCIDENTS].)*
...
### S · Staff Management & Compensation (👥 · platform · YENİ 2026-07-14)

> **Owner yönü (2026-07-14):** staff yönetimi ayrı, birinci-sınıf bir modül olmalı — çünkü personel **farklı ödeme modelleriyle** çalışıyor (kimi komisyon, kimi maaş, kimi self-employed/koltuk kirası). Hepsini tek yerde ayıran bir yapı gerek. **Bu platform (multi-tenant) işi**; whitecross'a özel `Finance.jsx` içine gömülmez (Finance şu an Tier 3, tenant-local — bkz S3). Sıra: I2 + Pre-Scale gate'in ALTINDA ama tanınan bir sonraki büyük modül.

**S1 — Veri-emniyeti temeli (silme güvenliği + snapshot)** · 🔵 **Sıradaki (küçük, çoğu hazır)**
Silme bariyerleri BÜYÜK ölçüde ZATEN VAR (audit 2026-07-14): silme yalnız super-admin+owner (UI `Barbers.tsx:179` + rules `firestore.rules:91`), onay modalı *"Set passive yap"* öneriyor (`Barbers.tsx:947`), `logAudit('BARBER_DELETED')` var. Bookings ismi snapshot'lıyor (`barberName` — walk-in `firestoreActions.ts:319`, online `BookingPage.tsx:673`) → silinse bile Reports/Finance isimle gösterir (`Reports.tsx:143` `d.barberName || lookup || raw`).
- ✅ **Delik 1 KAPANDI** (`0db230c`, 2026-07-14, PUSHED→CI): ürün satışı + blok artık `barberName` snapshot'lıyor (`createProductSale`/`blockTime`, createWalkIn ile parity) → barber silinse bile satışın ismi kalır. İleriye dönük, wage mantığına dokunulmadı; staff bundle rebuild dahil. (Geçmiş `barberName`'siz kayıtlar Reports'ta hâlâ `barberById` lookup'a düşer — canlı barber varken sorun değil.)
- **🔴 Kalan delik 2:** Reports → "Barbers" sekmesi listeyi sadece CANLI barber'lardan kuruyor (`Reports.tsx:182`) → silinen barber'ın geçmiş istatistik satırı komple yok olur. **Fix:** geçmiş booking'lerden çıkan barber isimlerini "Arşiv/eski personel" olarak dahil et.
- Öneri: hard-delete yalnız mükerrer/test kaydı için; gerçek ayrılma = passive (soft-delete, tüm geçmiş görünür kalır).

**S2 — Ödeme modeli / compensation (staff başına)** · 🟢 **FAZ B CANLI (2026-07-16) — UI + rules deploy edildi** · Faz C planlı
> **2026-07-16 CANLI:** Staff Hub Faz B tam yayında. UI: 12 commit + label commit push (`c1103af..b7208a7`) → CI hosting. Rules: `firebase deploy --only firestore:rules` CANLI (ruleset `1474907b`); protokol uygulandı (canlı çek→diff yalnız S1→test 95/95→deploy→re-fetch byte-aynı). staffComp read/write=owner+super; catch-all READ `{coll}/{document=**}`+`coll!='staffComp'` ile daraltıldı (OR-semantiği). Kapsam: compUtils+staffCompActions (append-only dönem matematiği, unit-testli — aynı-gün amend + reopen dahil) · PayModelChip (owner-only) + Former staff bölümü · sekmeli drawer (Profile/Availability/Pay/History) · 3 comp özet kartı + 3-adım CompChangeFlow (effectiveFrom min=bugün) · passive=comp-dönem-kapat · **wage periyotları hour|day|week|month|year + fiilî-çalışma tahakkuk semantiği** (owner kararı: çalışılan gün/saat; §2.1) · **paid-leave toggle** (owner-only, default OFF — barbershop gerçeği "çalışmadı→para yok"; salon isterse ücretli işaretler). Test toplam 59/59 + rules 95/95. Balance satırı "Tracked in Finance" (Faz C). Not: `whitecrossbarbers@`=tenantRole:staff → Pay sekmesi görmez (doğru); owner=`aerulas@`.
>
> **Faz C — KALAN (owner onayıyla):** M1 migration (partnerConfig→staffComp, dry-run CSV) · Finance staffComp'tan okuma + örtük £100 fallback kaldırma (parity kanıtıyla) · wage worked-time accrual motoru (hour..year gün/saat oranı) + paid-leave günleri normal oranla + commission booking-bazlı + chair-rent takvim tahakkuku · §7 emniyet fix'leri AYRI mini-koşu (occupancy resolver, legacy active-okuyucular→barberStatusOf, Reports arşiv=delik 2). Keep Scope Narrow.
> ✅ Tasarım dokümanı yazıldı: [STAFF_MANAGEMENT_DESIGN.md](STAFF_MANAGEMENT_DESIGN.md) (prompt çalıştırıldı, kod haritası satır-atıflı doğrulandı). Omurga: `tenants/{tid}/staffComp/{barberId}` (barbers world-readable olduğundan comp barber doc'una GİRMEZ) + append-only tarih-etkili `history[]` + "passive = comp dönemi kapalı" (bug 1 yapısal çözüm) + salt-türetim (booking alanlarına dokunulmaz). 3 faz: A veri modeli+göç (görünür değişiklik sıfır) · B Staff hub UI + emniyet fix'leri (occupancy bug 2 dahil) · C rapor entegrasyonu+platformlaşma. §8'de 4 açık owner sorusu var — kod öncesi cevaplanmalı.
Her personelin bir `compensation.type`'ı olmalı: **wage** (sabit £/gün-hafta-ay — bugünkü `partnerConfig`), **commission** (ürettiği cironun %'si; hizmet/ürün ayrı %), **self_employed** (koltuk kirası: sabit £/hafta VEYA cironun %'si dükkana). P&L'e etkisi üçünde bambaşka: wage→gider; commission→gelir−komisyon; self-employed→kişinin cirosu dükkan geliri DEĞİL, dükkan geliri=kira/komisyon (+ yasal ayrım: UK'de self-employed≠employee, vergi/istihdam riski). Bu yüzden comp tipi tek yerde tanımlı, Reports/Finance oradan türetmeli. **Kod öncesi tasarım dokümanı** (veri şeması + rapor kuralları + göç) yazılacak → hazır prompt: [STAFF_MANAGEMENT_DESIGN_PROMPT.md](STAFF_MANAGEMENT_DESIGN_PROMPT.md) (owner MacBook/Claude design'da çalıştıracak; çıktı buraya işlenecek).

**S3 — Lifecycle birleştirme + bilinen buglar** · 🟡 **Kısmen**
Durumlar: active / leave (tarihli, otomatik döner) / passive (kalıcı) / deleted. 2026-07-14 leave işleri: whitecross-site tarihe-duyarlı leave-hide CANLI (`f2ba207f`, INCIDENTS 2026-07-14). **Açık buglar (Finance whitecross-özel = Tier 3, aceleye getirme):** (1) passive barber Finance'ta hâlâ günlük maaş tahakkuk ediyor (`Finance.tsx:265` — leave kontrol var, passive/`barberStatusOf!=='passive'` filtresi yok); (2) izindeki barber occupancy kapasite paydasında sayılıyor (`OccupancyPanel.tsx` `barberWorksOn` leave-check'siz) → occupancy %'sini yapay düşürür. İkisi de S2 comp modeliyle birlikte temiz çözülür.

---

### B · Booking deneyimi (📅)

**B1 — Cancel/reschedule self-service UI** · ✅ **ZATEN CANLI — bayat rozet düzeltildi (2026-07-13 tespiti)**
`ManageBooking.tsx` 2026-06-14'ten beri canlı (`3d63c39`; off-day guard'ları `5476238`+`a98d39b` 06-29): `/manage/{tenantId}/{bookingId}` rotası, cancel + MiniCal'li reschedule tam akış, iki callable bağlı; TÜM tenant'ların confirmation/reschedule mailleri "Manage Booking" butonu taşıyor (bugünkü G1 Gmail-safe buton fix'i tam bu butonlardı). Canlı doğrulama: rota 200 + owner rc3+1'de uçtan uca test etti ("email→reschedule→cancel sağlam"). HeroHairs blocker'ı fiilen haziranda çözülmüştü. Kalan iyileştirmeler B2'nin işi (configurable pencereler, barber değişimi vs.).
...
3. **Faz 2 — Booksy browser robotu (Playwright)** · ✅ **KARAR VERİLDİ (owner, 2026-07-13 gece):** yapılacak — "Booksy inadı bırakıp verene kadar" geçici köprü; Booksy bir gün import/API açarsa robot emekli olur. Tasarım ADR'i ayrı oturumda yazılacak; **bağlayıcı tasarım şartları şimdiden:** (a) kimlik bilgisi YALNIZ Secret Manager, Firestore'a asla (H4 dersinin tersine dönüş yok) + salon başına açık rıza (onboarding'de imzalı toggle); (b) dar yetki: robot SADECE "Block time" basar/kaldırır — booking oluşturma/silme/müşteri verisi okuma YOK; (c) her robot aksiyonu auditLogs'a; (d) kill-switch (tenant flag'i kapatınca robot durur); (e) sessiz-kırılma YASAK: her run sonucu kaydedilir + fail'de alarm (backup dersi — sessiz cron en tehlikeli yalancı); (f) 2FA/CAPTCHA gerçeği tasarımda çözülür (kalıcı oturum çerezi saklama vs. her seferinde login) — pilotta whitecross kendi hesabıyla; (g) runner: Cloud Functions yerine izole Cloud Run job (puppeteer bellek/soğuk-başlangıç yükü). ToS riski owner'a açıkça anlatıldı ve kabul edildi (salonun kendi hesabı, kendi rızasıyla — "salonun kendi otomasyonu" çerçevesi). **(h) SINIR (owner, aynı gece): robot YALNIZ dışarı yön (slot kilitleme) — İÇERİ akış (booking ingestion) HER ZAMAN parser'da kalır** (parse-inbox önerilen + app-password IMAP legacy seçenek, ADR-015); robot hiçbir senaryoda booking kaynağına dönüşmez — iki sistem bağımsız, biri ölse öbürü etkilenmez.
**Gate:** Faz 0 sonucu görülmeden Faz 1'e başlanmaz. İlk pilot whitecross.
**❌ Faz 0 SONUÇ (2026-07-13 gece, masabaşı doğrulama — owner sezgisi + resmî yardım dokümanları):** İKİ platform da dış takvimi CANLI dinlemiyor — senkronları yalnız DIŞARI (Booksy→kişisel takvim, Fresha→kişisel takvim; Fresha "appointments will appear in your personal calendar", ~15 dk gecikmeli, gizlilik gereği detaysız). Booksy'nin "external calendar import"u tek-seferlik .ics dosya yükleme (walk-in'e çevirir) — abonelik değil. **GCal köprüsü (Faz 1) ÖLÜ.** Kalan yollar: (a) Faz 2 Puppeteer (bedelleriyle, ayrı ADR — tek zorlama yolu bu kaldı); (b) stratejik kabul: duvarlı bahçe bilinçli tasarım = "entegre olamazsın, koparırsın" — asıl cevap converted-client/direct-migration stratejisinin kendisi (H4 onların kapatamayacağı tek kapıdan [bildirim mailleri] zaten içeride). Owner'ın özeti: "diyor ki her şeyi ben yapıyorum zaten, başkasına ihtiyacın yok." Karar (Puppeteer'a girilir mi) sakin kafayla, ayrı oturumda.
**🎯 Faz 0 ek bulgu (aynı gece, owner Fresha panelinden — birincil kaynak):** Fresha'nın takvim-bağlama sihirbazında yön seçimi VAR ve **"Import events from an external calendar — Pull events from an external calendar URL into Fresha as blocked time" = COMING SOON.** Yani Fresha tam ihtiyacımız olan özelliği (URL'den blocked-time import) resmen geliştiriyor; yayınlandığı gün mevcut `salownIcalFeed?tenantId=X` yapıştırılır → **Fresha ayağı SIFIR kodla biter** (Treatwell modeli). Takip: Fresha release notes/panel ara ara kontrol (owner panelde görür). Booksy verdiği yok → Booksy için Puppeteer-veya-kabul kararı geçerli kalır. Güncel skor: Treatwell ✅ canlı · Fresha ⏳ coming-soon'a feed hazır · Booksy ❌ kapalı. **Yan kazanım:** Fresha'nın kendi EXPORT feed'i alındı (per-staff iCal, müşteri bilgisi gizli — sadece busy blok + appointment-id linki; Alex'inki `integrations.freshaIcalExportUrls`'ta). Booking kaynağı olarak zayıf (email daha zengin) ama **parser çapraz-kontrol/gap-filler adayı**: mailde olmayan booking feed'de görünürse alarm — I1 canary'nin doğal tamamlayıcısı; Treatwell `parseTreatwellIcalForTenant` modeli uyarlanabilir (ayrı küçük iş, gate'siz).
...
4. ✅ **C5-B — bulk yol da damgalıyor (2026-07-14, `1bf3416` + functions deploy):** C5/C5-A yalnız TEKLİ gönderimi (`sendMarketingEmail`) kapsıyordu; Campaigns redesign'ın bulk yolu (`sendCampaignBulk`) hiç damga yazmıyordu → whitecross lapsed30'a aynı "We miss you" blast'i 9 günde 3 kez gitti (04/06/13 Temmuz, 76/78/75 kişi). Artık bulk'ta her başarılı alıcıya `lastMarketingSentAt` (+lapsed/re-engage'te `reengagementSentAt` — BulkCampaignPanel lapsed segmentlerde `campaignType:'re-engagement'` yollar —, birthday'de `birthdayCampaignYear`) + `campaignsSent` log; doc'suz re-engage alıcısına C5-A find-or-create (TEK clients snapshot'a karşı eşleşir, per-alıcı scan yok). "Slipping away" kart sayacı compose ile aynı 30g suppress'i uyguluyor; insights "Send offer" geçersiz `'lapsed'` segment key'i (inSegment default'u → HERKESİ seçiyordu) → `'lapsed30'`. 13 Temmuz koşusunun 75 alıcısı geriye dönük damgalandı (script, 75/75 isim→doc). Residual: `all`/`haspoints` listelerinde "son N günde gönderilen" gizleme yok (veri hazır: `lastMarketingSentMs`). Detay: INCIDENTS 2026-07-06 KALAN #2.
...
**C10 — salOWN AI: doğruluk paketi + ürün bilgisi ("sitemap")** · 🟡 Doğruluk paketi ✅ CANLI (2026-07-14, `695a61f` → CI hosting + `functions:salown:askAI` deploy) · ürün-bilgisi 🔵 Planlandı (owner istedi 2026-07-14)
**Bağlam:** owner "13 Temmuz kasam?" sordu — AI 7 booking/£272 dedi, gerçek 9 booking/£276.60 (Firestore'dan doğrulandı). Kök neden: model son-30-booking satırlarını (kuruşsuz yuvarlanmış) KENDİSİ topluyordu (Haiku aritmetiği) + gün toplamı context'te hazır yoktu + "taking" tanımı belirsizdi + sohbet geçmişi gönderilmiyordu.
- ✅ **Doğruluk paketi (`1bd0885`):** buildContext'e DAILY TOTALS (son 14 işlem günü: service/tips/collected cash-card, 2 ondalık) + DEFINITIONS bloğu (service revenue vs collected/taking); booking satırları 2 ondalık; askAI'a sanitize'lı sohbet geçmişi (10 tur × 2000 kr cap — token-şişirme korumalı); max_tokens 512→1024; sistem prompt'a "gün sorusunu DAILY TOTALS'tan cevapla, satırları kendin toplama" kuralı.
- ✅ **Ürün bilgisi / sitemap CANLI (2026-07-14, `58668af` [lokal, push başka session'ın onayına bağlı] + `functions:salown:askAI` deploy):** `functions/src/ai/productGuide.ts` — kullanıcıya-dönük sitemap (panel + staff app + müşteri tarafı) + ~18 nasıl-yapılır + asistan notları (delete=owner-only, plan-gate). Prompt yönlendirme: how-to→SADECE rehberden (UI uydurmak yasak, bilinmeyende support'a yönlendir), veri→snapshot, cevap kullanıcının dilinde. ⚠️ Kaynak RAW ROADMAP/CLAUDE.md DEĞİL (iç doküman sızıntı riski) — curated rehber. **Bakım kuralı: user-visible feature çıkınca productGuide.ts'e satır ekle + askAI hedefli deploy.** Owner okuma turu ✅ ("iyi görünüyor, zamanla iyileştiririz" 2026-07-14). KALAN: feature-flag farkındalığı ve tool-use → C4.
...
**G1 — Minor** · 🔵
- ✅ **Email observability ÇÖZÜLDÜ** (2026-07-14, `56c8e5e` + 6 fonksiyon hedefli deploy CANLI): transactional sender'lar artık başarılı send'de booking doc'una stamp yazıyor — `confirmationEmailSentAt` (create + Stripe-confirmed trigger'ları, ortak helper) / `rescheduleEmailSentAt` (staff + self-service yolları) / `cancellationEmailSentAt` (self-service + panel callable, doc-id/bookingId çift lookup). "Mail gitti mi?" artık veriden okunur (loyalty `loyaltyEmailSent` paritesi, 07-13 yanlış-alarm sınıfı kapandı). Döngü kanıtı: update trigger'ları status GEÇİŞİ + `rescheduledAt` DEĞER DEĞİŞİMİ karşılaştırır → stamp no-op, çift mail yapısal imkânsız. Stamp'ler try/catch'li flag-only (patlasa mail akışı etkilenmez). 🔵 Opsiyonel takip: panel booking detayında "✉ sent" rozeti (frontend-only, ~10 dk).
- ✅ **`dailyFirestoreBackup` DÜZELDİ — günlük yedek ÇALIŞIYOR** (2026-07-13 akşam, owner onayıyla IAM verildi + CANLI DOĞRULANDI): iki izin eksikti → (1) runtime SA `1050766582653-compute@developer.gserviceaccount.com`'a proje-seviyesi `roles/datastore.importExportAdmin`, (2) Firestore service agent `service-1050766582653@gcp-sa-firestore…`'a `roles/storage.admin` @ `havuz-44f70.firebasestorage.app` bucket'ı. Scheduler job elle tetiklendi → `firestore-backups/2026-07-13/` + `overall_export_metadata` bucket'ta (export tamam). Bu gecenin 03:00 cron'u aynı izinlerle çalışır. Detay: [INCIDENTS.md](INCIDENTS.md) 2026-07-13. **Takip de aynı gün kapandı:** (a) 30-gün lifecycle silme kuralı bucket'a kuruldu — YALNIZ `firestore-backups/` prefix'i (bucket'taki `tenants/` upload'larına dokunmaz); (b) fonksiyona failure-alarm eklendi (`740916b` + hedefli deploy: export hatasında `info@salown.com`'a Brevo maili + run failed işaretlenir) — alarm yolu canlıda doğrulandı (aynı-gün ikinci export "Path already exists" → mail gitti).
...
- ✅ **Confirmation email buton bitişikliği ÇÖZÜLDÜ** (2026-07-13, `0d974f3` CANLI + hedefli functions deploy): "Add to Calendar" + "Manage Booking" `display:flex;gap` (Gmail desteklemez) → email-safe table düzeni, iki kopya da (G1 hızlı üçlü, bkz öncelik bloğu).
...
- ✅ **whitecross-site `parseBookingEmails` bounce-checker ÇÖZÜLDÜ** (2026-07-14, whitecross-site `62d79fe3` push + hedefli deploy `--only functions:whitecross:parseBookingEmails` [⚠️ codebase-prefix ŞART, prefix'siz "no match"] — **CANLI DOĞRULANDI:** 11:08'den itibaren ardışık 6+ koşu temiz "completed", sıfır hata): `parseBounceEmails` (2026-06-07, `4d529b12`) `fetchRecentMessages`/`extractPlainText`/`extractHtmlAsText` kullanıyordu ama bunlar `emailParsers.js`'ten hiç export/import edilmemişti → scheduler 5 hafta boyunca 5 dk'da bir `ReferenceError`, loyalty bounce flag'leri hiç yazılmadı. Fix = 3 helper export + import (6 satır, davranış değişikliği yok). Bundan sonraki mailer-daemon bounce'ları `loyaltyEmailBounced` yazacak; geriye dönük yalnız SON 7 GÜN yakalanır (`fetchRecentMessages` `after:` 7-gün penceresi) — 5 haftalık kırık dönemin eski bounce'ları işlenmez (kabul edilen kayıp; gerekirse tek-seferlik backfill script'i).
...
- **Panel in-app notification: müşteri/parser kaynaklı reschedule+cancel görünmüyor** (2026-07-13, owner H4 canlı tatbikatında fark etti): Booksy'den müşteri reschedule/cancel yapınca Telegram mesajı geliyor (mantığı güzel — ne değişti gösteriyor) ama panel zil/in-app notification HİÇ almıyor (in-app yalnız yeni-CONFIRMED'de var, `salownNotifyBookingCreated`). İş: (1) `salownNotifyBookingUpdated`'dan Telegram'a paralel in-app notification doc'u da yaz — Telegram'daki "ne değişti" mantığıyla (eski→yeni tarih/saat/barber; cancel'da CANCELLED); (2) bildirime tıklayınca ilgili booking'in detay/reschedule penceresi açılsın (owner: legacy'de vardı). İki boru da (IMAP+inbound) aynı trigger'ı beslediği için ingestion'dan bağımsız çalışır. (3) **Kişi-bazlı bildirim tercihi** (owner, aynı gün): olay-toggle'ları tenant-geneli (Settings→Notifications 5 event + 5 client-email, dinamik ✓) ama individual tercih YOK — push tüm cihazlara gidiyor. Temel hazır: `fcmTokens` doc'ları zaten `uid`/`barberName`/`role` taşıyor → staff-app'e tercih ekranı + gönderimde token filtresi (örn. staff yalnız kendi barber'ının olaylarını alsın; permissions canView* mantığının bildirim karşılığı). Ayrıca Settings'teki event-toggle metni "in-app dahil" diyor ama cancel/reschedule in-app HİÇ üretilmiyor — (1) yapılınca vaat gerçekle buluşur.
- EeKurt legacy site → salown subdomain redirect
- `categoryId` migration
- Dead `isStaff` Firestore rule
- ✅ **Barbers `cycleStatus` leave koruması + barber-audit ÇÖZÜLDÜ** (2026-07-13, `b582042` CANLI): on-leave üyede "✓ Activate" artık confirm soruyor + `BARBER_STATUS_CHANGED` audit kaydı (INCIDENTS 2026-07-12 Muhamed vakası kapanışı; I4'ün ilk dilimi).
- ✅ **"Send Finish your booking" confirm'i ÇÖZÜLDÜ** (2026-07-13, `b582042` CANLI): confirm modal + `ABANDONED_CART_SENT` audit kaydı (INCIDENTS 2026-07-12 Guru vakası kapanışı).
...
**2. GERÇEK KURBAN (2026-07-13, HeroHairs):** owner Salı'yı "her yerden" açtı (workingDays ✓ + shift-override ✓) ama `dayHours.Tuesday.closed:true` hiçbir yüzeyden temizlenmedi → walk-in "no barbers available" (`bookingUtils.ts:171` eler). Tek-alan veri düzeltmesiyle çözüldü (closed→false, admin SDK). ✅ **Adım 2a-ek: PUBLIC PROJEKSİYON KATMANI (2026-07-13, `81f2824` CANLI+kanıtlı):** 5. yüzey bulundu — saat düzenlemeleri `settings/*`'ta yaşarken republish trigger'ı yalnız tenant KÖK doc'unu izliyordu → salon sayfası/premium site müşteriye bayat saat gösteriyordu (HeroHairs Salı: panel açık, sayfa 'kapalı' dedi; elle republish edildi). Kalıcı fix: `salownRepublishOnSettingsEdit` (53. fonksiyon, settings/{settings|hours} update'inde selfManaged+published tenant'ı yeniden yayınlar) — canlı test: değişiklikten 4 sn sonra projeksiyon tazelendi. ⚠️ Test dersi: Firestore NO-OP yazma (aynı değer) update event'i ÜRETMEZ — trigger testi gerçek değer değişikliğiyle yapılır.
✅ **Adım 2a CANLI (2026-07-13, `282e5ae`):** resolver artık shiftChange override'ını tanıyor (açık override bayat closed'u yener — dört panel formu otomatik düzeldi) + Barbers kaydı workingDays'teki günlerin closed bayrağını otomatik temizliyor (çelişkili durum yazılamaz). ✅ **Adım 2b+3 CANLI (2026-07-13, `ca82f76`):** Dashboard day-view izindeki barber'a kolon çizmiyor (o güne booking'i varsa kolon KALIR — INC 06-29 dersi) + BookingPage `where(active==true)` bıraktı → izindeki personel sitede seçilebilir, izin günleri slot üretmez, **izin bitince OTOMATİK döner** (Muhamed 20 Ağu'da kendiliğinden online'a düşecek). ✅ **Adım 4 CANLI (2026-07-13 gece, `2af65a0` + hedefli deploy):** `salownRescheduleByToken` off-day guard'ına leave+passive kontrolü — müşteri email-linkiyle izinli/pasif barber'a reschedule SERVER'da reddedilir (frontend resolver'ın birebir aynası: passive→leave `[leaveFrom,leaveUntil]`→shiftChange→workingDays/dayHours; canlı resolver'daki gibi leave açık shift-override'ı yener). Whitecross premium sitesi dahil TÜM yüzeyleri kapsar (`Reschedule.html:684` aynı callable'ı çağırır). ⚠️ Yan bulgu: create yolunun server garantisi hâlâ YOK (BookingPage doğrudan Firestore yazar → **B3** transactional'ın kapsamı, INV-BK-1). ✅ **whitecross-site resolver portu CANLI (2026-07-14, whitecross-site `bc2f98ef` + saas hosting deploy, curl doğrulandı):** `script.js`+`Reschedule.html` yalnız `active===false` okuyordu (izin bitince OTOMATİK DÖNMEZ + gelecek tarihli izinde erken düşer) → `barberStatusOf`+`isBarberOnLeaveForKey` (panel aynası, tarih-aralıklı) ile barber görünürlüğü + per-date slot üretimi + Reschedule barber grid'i date-aware; Muhamed 20 Ağu'da whitecrossbarbers.com'da da kendiliğinden döner. ⚠️ Semantik ayrışma NOTU: whitecross-site'ta özel gün override'ı izni YENER (`115133c4` bilinçli), panel/server-guard'da leave kazanır — izin aralığı İÇİNE atanan açık özel gün edge'i adım 5'te tek semantiğe bağlanmalı (owner kararı). **Adım 5 durumu (2026-07-14 incelemesi):** E'nin görünür-durum kısmı ZATEN YERLEŞİKMİŞ (Barbers roster `statusEff` biten/başlamamış izni 'active' okur + "Leave · <tarih>" rozeti; availability yüzeyleri pencereli `isBarberOnLeaveForDate`; confirm+audit `b582042`) → adım 5'ten gerçekte kalanlar: **(a) semantik birleştirme — ✅ OWNER KARARI VERİLDİ (2026-07-14 gece): OVERRIDE KAZANIR.** Gerekçe: "biz o gün gelecek demişiz; gelmezse edit yapıp erkene/ileriye alırız." Tek doğru öncelik (audit §4A ile aynı): `shiftChanges (açık) > leave > passive > workingDays/dayHours` — izin içine girilen açık özel-gün o günü ÇALIŞIR yapar ve o gün ÜCRETE SAYILIR (geldi=çalıştı=para). **✅ UYGULANDI (aynı gece, `e68dca8` → CI + `salownRescheduleByToken` hedefli deploy; +3 test, iki taraf strict/build temiz):** beş yüzey aynı önceliğe çekildi — whitecross-site ile semantik ayrışma KAPANDI. Orijinal harita (arşiv):** (1) `bookingUtils.getAvailableBarbersForDate` — sc kontrolü leave'in ÖNÜNE (sc varsa `!sc.closed` döner, leave'e bakılmaz); (2) Dashboard grid leave-gizleme (`Dashboard.tsx:413` civarı) — o güne açık sc varsa kolon ÇİZİLİR; (3) server guard `salownRescheduleByToken` (adım 4 bloğu) — açık sc leave'i yener, kapalı sc bloklar; (4) Finance 5 sayaç + G4 haftalık ledger — sıra `startDate → sc(kapalı=hayır/açık=EVET) → leave → wdays` (bugünkü sıra leave-önce: gelinen gün ücrete SAYILMIYOR = eksik maaş — düzeltilmeli); (5) whitecross-site zaten böyle ✓. Arşiv leave-history ile uyumlu (sayaç sc-önce bakınca arşivli aralık içindeki açık-sc günü yine sayılır). **(b2) ✅ 5. SAYAÇ YAKALANDI+FİX CANLI (2026-07-14, `4b7b592` → CI; owner canlıda fark etti — Muhamed'in ilk izin günü):** `c66320d` 4 aylık/haftalık sayacı korumuştu ama Finance'in GÜNLÜK P/L satırı wage'i bağımsız hesaplıyor ve leave guard'ı yoktu → bugünün satırı Muhamed'in £100'ünü saydı (aylık toplam doğruydu). Aynı guard aynı sırayla eklendi. **(b) ✅ leave-history CANLI (2026-07-14, `3898eb0` → CI; staff-session push'uyla birlikte gitti):** izin tarihleri tek alan çiftindeydi → sonraki izin girilince/activate'te eski aralık siliniyor, Finance/G4 all-time ledger geçmiş izin günlerini yeniden sayıyordu (Muhamed Tem-Ağu → Eylül'de bakiye kayması). Fix: `barber.leaves[]` arşivi — `archiveLeaveRange` (erken biten izni DÜNe kadar kırpar [yeni aktifleşen bloke kalmasın], hiç başlamamışı düşürür, dedup) + `isBarberOnLeaveForDate` arşiv aralıklarını da sayar (arşiv ≤dün olduğu için bugün/gelecek availability'ye etkisi SIFIR; Dashboard geçmiş-gün görünümü de tarihsel doğru oldu). Yazım noktaları: Barbers save + cycleStatus. 12 unit test, strict+build temiz. KALAN: adım 6 Staff hub (L, ayrı brief) + staff-app geçişi (diğer cihaz koordinasyonu).
...
- ✅ **H3c — Parse Inbox Address yönetimi (UI) CANLI (2026-07-13, `salown-admin.web.app`, salownadmin `a31538f`).** Tenant çekmecesine "Parse Inbox Address" bölümü: mevcut adresi (`settings/integrations.parseInboxAddress`) monospace+Copy göster; **Generate/Rotate** (`salownProvisionParseAddress`, rotate'te confirm — "forwarding güncellensin") + **Revoke** (`salownRevokeParseAddress`, confirm). UI-only, yeni fonksiyon yok (callable'lar H4'te yazılıp deploy edildi — canlı doğrulandı). Audit-log kaydı. **H4'ün (2b) super-admin token yüzeyi bu** → H4 token üretimi artık panelden yönetilebilir.
...
**H4 — Parser mail girişi: parse-inbox hybrid + token izolasyon** · ✅ **PİLOT TAM CANLI (2026-07-13/14)** — forwarding kuruldu + gerçek maille tam yaşam-döngüsü tatbikatı GEÇTİ (create/reschedule/chain/cancel × iki boru, sıfır çift kayıt; detay aşağıda). ✅ **İLK ORGANİK MÜŞTERİ MAİLİ GÖZLENDİ (2026-07-14 07:28Z):** "Rory Breach: new booking Tue 14 July 09:00" boru üzerinden uçtan uca aktı → `PARSED`, dedup `skipped=1`, sıfır çift kayıt. Yarışı bu kez IMAP kazandı (import onda; boru dedup'la tanıdı) — iki boru paralel koştukça yarış sonucu değişken, önemli olan kanıt: **organik mailde routing+parse+dedup üçü de doğru çalıştı.** Boru'nun `imported=1` ile kazanması IMAP emekliye ayrılınca kendiliğinden norm olur. ✅ **Fresha/Treatwell forward filtreleri KURULDU + FRESHA BORUSU CANLIDA KANITLANDI (2026-07-14 16:52Z):** owner iki filtreyi ekledi (`from:(fresha.com)` + `from:(noreply@treatwell.co.uk)` → aynı bk_ adresi); gerçek Fresha test booking 4 sn'de PARSED, `imported=1` — **yarışı boru kazandı, booking'i boru yarattı** (Booksy organiğinde IMAP kazanmıştı; iki yön de kanıtlı), karantina temiz. Kalan: Treatwell borusu ilk mail gözlemi + herohairs geçişi (token rotate ✅, forwarding kurulumu yeni adresle: `herohairs_2e1355b0…@parse.salown.com`) + model oturunca whitecross IMAP emekliliği (owner istekli — 5-dk cron sorgu yükü; yöntem: app-password kaldır, feature flag'lere DOKUNMA [iki boruyu birden gate'liyor]; T-b buharlaşır). ⚠️ Ev işi (2026-07-14, log ile YENİDEN doğrulandı — hâlâ sürüyor, son gözlem 09:38Z): tatbikatın 3 test maili whitecross Gmail'de UNSEEN kalmış → IMAP cron 5 dk'da bir aynı "Booksy cancel/reschedule not found" üçlüsünü yeniden işleyip logluyor (zararsız — yazma yok — ama gürültü + gerçek hatayı maskeler); çözüm: owner o 3 maili okundu işaretlesin YA DA parser'a terminal not-found'da mark-seen kuralı (dikkat: out-of-order retry davranışını bozmadan)
> **Kapanış (2026-07-13):** (1) **Parse dispatch CANLI** (`c944b28` + hedefli deploy): parser per-mesaj mantığı `parse<X>Messages`'a çıkarıldı (saf kod taşıma; IMAP yolu = fetch→aynı pipeline→mark-seen) + yeni **`salownParseInboxDispatch`** trigger'ı (parseInbox PENDING → feature-flag gate [IMAP ile AYNI flag'ler] → parse → `recordParserRun` [ortak I1 canary] → PARSED + rawEmail silinir [boru-değil-depo]; ERROR'da raw kalır = replay). **Fork-yok kanıtı:** `messages.test.js` karakterizasyon testleri — 3 platformda da iki yol BİREBİR aynı Firestore yazımlarını üretiyor (41/41). IMAP cron refactor sonrası canlıda 0 hata. (2) **DNS+Brevo BİTTİ:** `parse.salown.com` MX (inbound1/2.sendinblue.com) + brevo-code TXT + parse DKIM CNAME'leri; domain Brevo'da authenticated; **inbound webhook kayıtlı (id 2080372, `?key=` secret'lı)**. ⚠️ Kurulumda salown.com GÖNDERİM DKIM CNAME'leri yanlışlıkla silindi → geri kondu + Brevo yeniden-doğrulama GEÇTİ + noreply test maili atıldı (ders: `brevo1._domainkey` [gönderim] ≠ `brevo1._domainkey.parse` [alım], DNS düzenlerken karıştırma). (3) **Token'lar üretildi** (Firestore REST, callable şemasıyla): whitecross `bk_b5ad432b…` + herohairs ~~`bk_2d01853d…`~~ → **ROTATE 2026-07-14: `herohairs_2e1355b0…`** (owner isteğiyle token formatı artık `<salonslug>_<32hex>` — adresten salon okunur; mint+routing `1183f50` + hedefli deploy CANLI, legacy `bk_` desen çalışmaya devam ediyor, whitecross'a dokunulmadı; herohairs forwarding'i kurulurken YENİ adres kullanılacak) — tam adresler `tenants/{id}/settings/integrations.parseInboxAddress`. (4) **İki canlı test GEÇTİ:** pipe (mail→MX→Brevo→webhook→whitecross parseInbox staged→dispatch UNKNOWN_SOURCE kararı doğru) + izolasyon (bilinmeyen token→`superAdmin/parseQuarantine` `unknown_token`, HİÇBİR tenant'a yazılmadı — fail-closed CANLIDA kanıtlı). **KALAN:** ✅ o da bitti — whitecross Gmail forwarding kuruldu (adres doğrulandı, filtre `from:no-reply@booksy.com`; genel forwarding BİLİNÇLİ kapalı) ve **İLK GERÇEK BOOKSY MAİLİ zinciri doğruladı (2026-07-13 ~20:15Z):** owner'ın test booking'i → forward boruya IMAP'ten 2 dk ÖNCE ulaştı ama `no_token` karantinasına düştü → kök neden: Gmail filter-forward orijinal `To:` başlığını korur, token yalnız ZARFTA (Brevo `Recipients`) — kod başlığı tercih ediyordu; ayrıca Brevo çıplak gövde gönderir, parser'lar ham MIME bekler. **Fix CANLI (`0b829ba` + deploy):** zarf-öncelikli routing (gövdeye ekilen sahte token yine kaybeder, testli) + minimal MIME zarfı yeniden kurulur (Subject/From/Date/To; To en sonda — extractor lookahead kuralı). Aynı gerçek mail replay edildi: 4 sn'de `PARSED`, `skipped=1` (IMAP'in yazdığı booking'i dedup tanıdı, çift kayıt YOK) = routing+parse+dedup üçü birden gerçek mailde kanıtlı. Karantina fail-closed sınavı da geçti (yanlış tenant'a sıfır yazım). **✅ TAM YAŞAM-DÖNGÜSÜ TATBİKATI GEÇTİ (2026-07-13 gece, owner gerçek Booksy hesabıyla):** create (IMAP kazandı→replay dedup kanıtı) → reschedule saat (BORU kazandı, 1 sn, updated=1) → CHAIN reschedule tarih+saat+barber üçü birden (IMAP kazandı, boru idempotency skip) → CANCEL (BORU kazandı, updated=1→CANCELLED; iptal maili taşınmış tarihi andı, doc ID eskiydi → tarih/saat fallback buldu). Dört yol × iki boru × iki yarış sonucu = sıfır çift kayıt, sıfır hayalet. Model oturunca **T-b buharlaşır**. Fresha/Treatwell filtreleri owner'da opsiyonel bekliyor. Tatbikatta çıkan yan iş: panel in-app notification maddesi (G1'e eklendi). *(Aşağısı = orijinal plan, arşiv.)*
...
**I1 — Parser sessiz-kırılma canary'si** · ✅ **CANLI (rozet düzeltildi 2026-07-14, canlı veriyle doğrulandı)** — `recordParserRun` İKİ boruda da çağrılıyor (IMAP cron `src/index.ts:1764-1766` + `salownParseInboxDispatch:1848`), H4 dispatch paketiyle (`c944b28`) deploy edilmişti; `tenants/whitecross/parserStats/{booksy|fresha|treatwell}` 2026-07-08'den beri günlük sayaç yazıyor (lastRunAt 2026-07-14 10:28Z, Firestore REST ile okundu). Eski "deploy edilmedi" rozeti bayattı.
...
**I2 — `functions/src/index.ts` split** · 🟡 **Faz 1 fiilen BİTTİ (TS migration'da) — Faz 2 SIRADA (owner seçimi 2026-07-14)** · GATE: sonraki feature'dan ÖNCE (owner kararı 2026-07-04)
**Envanter güncellemesi (2026-07-14):** eski `index.js` (5562 satır) migration'da `src/index.ts`'e dönüştü; helpers zaten domain modüllerinde (Faz 1 ✓, parity testli). Kalan: **index.ts 3816 satır, 55 export'un tamamı hâlâ orada tanımlı** → Faz 2 = fonksiyon gövdelerini domain modüllerine taşı.
✅ **Dilim 1 CANLI (2026-07-14, `bccd828` + `functions:salown:askAI` hedefli deploy):** askAI → `src/ai/askAI.ts` (config birebir, index.ts re-export) + **auth guard** — public callable ANTHROPIC_API_KEY yakıyordu, artık `unauthenticated` reddi (canlı curl kanıtlı; SECURITY askAI açığı kapandı). Kanıt: strict 0 · lib export parity 55/55 · test 46/0 · deploy UPDATE. Sıradaki dilimler: parsers → notifications → marketing; **stripe/bookings EN SON**.
...
**I4 — Audit trail'i TAM kapsama çıkar (her mutasyon loglanır)** · 🟡 **FAZ A CANLI (2026-07-13, `2ab0328` → CI):** personel/rota + client-kimlik yazarları auditli — CLIENT_CREATED/UPDATED(diff)/DELETED(sayaçlarla)/CONSENT_CHANGED/MEMBERSHIP_CHANGED + BARBER_CREATED/UPDATED(diff)/DELETED/SHIFT_OVERRIDE (+önceki dilimler: BARBER_STATUS_CHANGED, CLIENT_TRUSTED_CHANGED, ABANDONED_CART_SENT; CLIENTS_MERGED zaten vardı). logAudit artık source:'panel' damgalıyor. KALAN: Faz B (katalog/fiyat + settings + discount codes) + Faz C (staff-user fn'leri, super-admin, TTL, viewer filtreleri, append-only rules).

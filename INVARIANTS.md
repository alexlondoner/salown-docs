...
| INV-BK-6 | Slot generation: SSOT = the staffer's **effective shift** (`getEffectiveStaffShift`, `src/utils/staffAvailability.ts`); salon hours are a FALLBACK only and never clip an explicit shift. The service must **fit inside the shift**: `slotStart + serviceDuration <= effectiveStaffShiftEnd` (`generateStaffSlots`). Priority: date-specific override > leave > passive > weekly > salon fallback. (**Superseded 2026-07-23** the old "last start = close−15min, spillover allowed" rule — do NOT reintroduce spillover into the availability path; compute analytics separately if needed.) | Salon close clips a real shift, or a service spills past the shift end | 🟡 | CLAUDE §Slot · owner decision 2026-07-23 |

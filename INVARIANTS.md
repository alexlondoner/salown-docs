...
| INV-PARA-14 | **`barbers.workingDays` is undated — it is NOT a historical record.** Every historical Finance month is recomputed from today's rota. ⚠️ **Corrected 2026-08-18:** this row used to read *"Until S3's dated compensation periods land…"*. S3 **did** land and is `LIVE_VERIFIED`, and the invariant is **unchanged** — `effectiveFrom`/`effectiveTo` dates WHETHER somebody was employed and never WHICH DAYS they worked. It is retired by `FIN-ROTA-HISTORY-READ` reaching `'dated'` **and** INV-PARA-15, not by S3. Do not "repair" a rota to fix a past total, and do not encode an employment end as a fake leave or a `wageEndDate` field | A rota edit silently restates closed, already-reported and already-settled months; a second employment-end SSOT that nothing reconciles | 🟠 | INC 2026-08-12 · ROADMAP **S3** |
| INV-PARA-15 | **A CLOSED month is STORED, not derived.** Once `financePeriods/{YYYY-MM}` exists and `FINANCE_PERIOD_CLOSE_MODE` is `'closed'`, that month's figures come from the snapshot — never from a recomputation. The base is **write-once**: there is no reopen, no update path and no delete path for it in the product, and `firestore.rules` denies every browser write including super-admin's | A rota edit, an expense correction, a re-opened booking or a back-dated `shiftChanges` key restates a month that was already reported — and, on this tenant, already SETTLED. It has happened twice for real money (≈£12,300 on 2026-08-13; +£1,800 on 2026-08-16) | 🔴 | `src/utils/financePeriodClose.ts` · `functions/src/finance/periodClose.ts` · [FIN_PERIOD_CLOSE_DESIGN](FIN_PERIOD_CLOSE_DESIGN.md) |
| INV-PARA-16 | **A post-close correction is an APPENDED, ATTRIBUTABLE adjustment** — actor, reason, `evidenceRef`, before/after/delta, and a **stable staff id** for any wage move. It may never target a derived total (`netPL_m`, `wagesTotal_m`) and never rewrites the base. It is folded into the closed month's effective view **exactly once**; the current month's prior-period line is a **memo** that no total accepts as an argument | The record stops being a record: the number a settlement was signed against is overwritten with no trace, or the same correction is counted twice in all-time. Same reasoning as INV-PARA-9 | 🔴 | `effectivePeriodTotals` · `composeAllTimeTotals` · [FIN_PERIOD_CLOSE_DESIGN](FIN_PERIOD_CLOSE_DESIGN.md) §8 |
| INV-PARA-17 | **A production period close is gated by a SERVER-OWNED release allowlist, above and independent of the calendar.** `PRODUCTION_PERIOD_RELEASE_POLICY` in `functions/src/finance/periodClose.ts` names the exact (tenant, period) pairs a production call may name — today `whitecross` **2026-02 … 2026-07** and nothing else — and it is checked BEFORE the tenant document is read, for **dry run, apply and adjust alike**. Refusal is the stable `PERIOD_NOT_RELEASE_AUTHORISED`. It may never come from the request body, the UI, tenant settings, a public Firestore document or an environment value; the only test hook is emulator-gated and unreachable from the callable | `CURRENT_PERIOD_OPEN` is a CALENDAR answer and it changes by itself: when September begins in Europe/London, August 2026 becomes historically eligible and a calendar-only gate starts accepting a month nobody released. An unauthorised production DRY RUN is also how an apply gets staged — it is what produces the `planDigest` the apply must carry back | 🔴 | `functions/src/finance/periodClose.ts` · `periodClose.test.js` §RA · [FIN_PERIOD_CLOSE_DESIGN](FIN_PERIOD_CLOSE_DESIGN.md) |
| INV-PARA-18 | **The period-close `tenantId` SELECTS a target and is never authority evidence.** Only the single TOP-LEVEL field is a selector; a nested, duplicated or renamed one is refused. Authority comes from the verified identity, the `superAdmin` claim, the stored staff document, the role re-read INSIDE the write transaction, the release allowlist, the approved plan digest, source-drift validation and the transactional preconditions. A non-super-admin may PREVIEW only when `request.tenantId === auth.token.tenantId`; apply and adjust are super-admin-only | A caller-supplied field becomes a privilege: an owner selects somebody else's salon, or a second selector deeper in the payload contradicts the one that was checked. Recorded as an owner contract amendment on 2026-08-19 because a super-admin token carries **no** tenant and no operator→tenant binding exists to derive one from | 🔴 | `decidePeriodCloseAuth` · `PERIOD_CLOSE_REQUEST_KEYS` / `PERIOD_CLOSE_FORBIDDEN_KEYS` · `periodClose.test.js` §11 |
| INV-PARA-19 | **Finance buckets an instant in the TENANT's calendar, never the reader's device calendar.** There is exactly ONE timezone source (the TR-A `presentation` contract via `useLocale()`); Whitecross is Europe/London, so a reader in any other zone receives London day and month boundaries. ⚠️ Therefore **universal byte-for-byte legacy parity across browser timezones is NOT claimed**: parity is exact where the two calendars already agreed, and an authoritative correction where they differed. App-wide `toDateKey` is deliberately outside this rule | A booking at 23:30 on 31 July London is filed into August for a reader in Istanbul and July for a reader in London — two people disagree about which CLOSED MONTH a booking belongs to, and month eligibility is decided for the wrong period | 🟠 | `src/pages/Finance.tsx` · `financePeriodCloseReader.test.ts` §C5/§C5b |
